...
//
//  override lazy val db = swaydb.memory.Map[Long, Domain]().unsafeGet
//}
//
//class Memory_SimulationGroupingStrategySpec extends SimulationSpec {
//
//  override lazy val db = swaydb.memory.Map[Long, Domain](groupingStrategy = Some(DefaultGroupingStrategy())).unsafeGet
//}
//
//class Persistent_SimulationSpec extends SimulationSpec {
//
//  override lazy val db = swaydb.persistent.Map[Long, Domain](randomDir, acceleration = Accelerator.brake()).unsafeGet
//}
//
//class Memory_Persistent_SimulationSpec extends SimulationSpec {
//
//  override lazy val db = swaydb.persistent.Map[Long, Domain](randomDir, mmapAppendix = false, mmapMaps = false, mmapSegments = MMAP.Disabled).unsafeGet
//}
//
...
//
//        db.put(productId1, product1).unsafeGet
//        db.put(productId2, product2).unsafeGet
//        db.put(productId3, product3).unsafeGet
//
...
//          db.put(productId, putProduct).unsafeGet
...
//
//          if (randomBoolean)
//            db.update(productId, updatedProduct).unsafeGet
//          else {
//            val functionID =
//              db.registerFunction(
...
//
//          if (randomBoolean)
//            db.expire(productId, newDeadline).unsafeGet
//          else {
//            val functionID =
//              db.registerFunction(
//                functionID = functionIDs.incrementAndGet(),
...
//              batchProduct =>
//                db.unsafeGet(batchProduct._1).unsafeGet should contain(batchProduct._2._1)
//            }
//          catch {
//            case ex: Exception =>
//              ex.printStackTrace()
...
//            }
//          db.put(batchUpdatedProducts).unsafeGet
//
...
//
//          //update a random single product
//          db.expire(batchExpire).unsafeGet
//          batchExpire foreach {
//            case (productId, newDeadline) =>
//              val (product, _) = state.products.unsafeGet(productId).unsafeGet
//              state.products.put(productId, (product, Some(newDeadline)))
//          }
//        }
//
...
//
//          if (randomBoolean)
//            db.update(from, to, updatedProduct).unsafeGet
//          else {
//            val functionID =
//              db.registerFunction(
//                functionID = functionIDs.incrementAndGet(),
...
//          (from to to) foreach {
//            updatedProductId =>
//              state.products.unsafeGet(updatedProductId) map {
//                case (_, deadline) =>
...
//
//          if (randomBoolean)
//            db.expire(from, to, newDeadline).unsafeGet
//          else {
//            val functionID =
//              db.registerFunction(
//                functionID = functionIDs.incrementAndGet(),
...
//
//          if (randomBoolean)
//            db.remove(productToRemoveId).unsafeGet
//          else {
//            val functionID =
//              db.registerFunction(
//                functionID = functionIDs.incrementAndGet(),
...
//          db.remove(batchRemove).unsafeGet
...
//        db.unsafeGet(state.userId).unsafeGet should contain(state.user)
...
//              Try {
//                Await.result(Future(db.unsafeGet(productId).unsafeGet should contain(product._1)), 10.seconds)
//              } recoverWith {
//                case ex =>
...
//              Try {
//                Await.result(Future(db.unsafeGet(productId).unsafeGet shouldBe empty), 10.seconds)
//              } recoverWith {
//                case ex =>
...
//
//    "print DB" in {
//      println(db.unsafeGet(160000000000001887L).unsafeGet)
//    }
//
