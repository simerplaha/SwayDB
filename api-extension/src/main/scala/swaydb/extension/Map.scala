...
//                                valueSerializer: Serializer[V]) extends MapIterator[K, V](mapKey, dbIterator = map.copy(map.core, Some(From(Key.MapStart(mapKey), orAfter = false, orBefore = false, before = false, after = true)))) {
//
//  def maps: Maps[K, V] =
...
//    map.unsafeGet(Key.MapStart(mapKey)).map(_.flatten)
//
...
//    */
//  def unsafeGet(key: K): IO[Option[V]] =
//    map.unsafeGet(Key.MapEntry(mapKey, key)) flatMap {
//      case Some(value) =>
//        IO.Success(value)
//      case None =>
//        IO.none
