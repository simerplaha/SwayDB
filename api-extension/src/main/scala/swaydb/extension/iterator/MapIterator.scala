...
case class MapIterator[K, V](mapKey: Seq[K],
                             ...
                             dbIterator: swaydb.Map[Key[K], Option[V], IO],
  ...
  private def before(key: Key[K], reverse: Boolean): MapIterator[K, V] =
    copy(dbIterator = dbIterator.before(key).copy(reverseIteration = reverse))

  private def reverse(reverse: Boolean): MapIterator[K, V] =
    copy(dbIterator = dbIterator.copy(reverseIteration = reverse))
  ...
  override def iterator: Iterator[(K, V)] = {
    ...
    //            mapKey match {
    //              case Key.MapStart(_) =>
    //                if (dbIterator.reverseIteration) //exit iteration if it's going backward since Start is the head key
    //                  false
    //                else
    //                  hasNext
    //
    //              case Key.MapEntriesStart(_) =>
    //                if (dbIterator.reverseIteration) //exit iteration if it's going backward as previous entry is pointer entry Start
    //                  false
    //                else
    //                  hasNext
    //
    ...
    //                if (mapsOnly) {
    //                  if (dbIterator.reverseIteration) //Exit if it's fetching subMaps only it's already reached an entry means it's has already crossed subMaps block
    //                    false
    //                  else
    //                    hasNext
    //                } else {
    ...
    //                if (!dbIterator.reverseIteration && !mapsOnly)
    //                  false
    //                else
    //                  hasNext
    //
    ...
    //                  if (dbIterator.reverseIteration) //if subMaps are excluded & it's going in reverse continue iteration.
    //                    hasNext
    ...
    //                if (!dbIterator.reverseIteration)
    //                  false
    //                else
    //                  hasNext
    //
