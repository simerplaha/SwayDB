...
//  override val db = swaydb.memory.Map[Int, WeatherData]().unsafeGet
//}
//
//class Memory_WeatherDataGroupingStrategySpec extends WeatherDataSpec {
//  override val db = swaydb.memory.Map[Int, WeatherData](groupingStrategy = Some(DefaultGroupingStrategy())).unsafeGet
//}
//
//class Persistent_WeatherDataSpec extends WeatherDataSpec {
//  override val db = swaydb.persistent.Map[Int, WeatherData](randomDir, cacheSize = 10.mb, acceleration = Accelerator.brake()).unsafeGet
//}
//
//class EventuallyPersistent_WeatherDataSpec extends WeatherDataSpec {
//  override val db = swaydb.eventually.persistent.Map[Int, WeatherData](randomDir, maxOpenSegments = 10, cacheSize = 10.mb, maxMemoryLevelSize = 500.mb).unsafeGet
//}
//
...
//        db.put(key, WeatherData(Water(key, Direction.East, key), Wind(key, Direction.West, key, key), Location.Sydney)).unsafeGet
//    }
//
...
//    (1 to keyValueCount) foreach {
//      key =>
//        val value = db.unsafeGet(key).unsafeGet
//        if (key % 10000 == 0)
...
//        db.remove(key).unsafeGet
//    }
//
//    db.remove(keyValueCount / 2, keyValueCount + 1).unsafeGet
//  }
//
