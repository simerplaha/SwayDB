...
//    */
//  def getValue(): IO[Core.Error.Private, Option[V]] =
...
//    */
//  def get(key: K): IO[Core.Error.Private, Option[V]] =
...
//    */
//  def getKey(key: K): IO[Core.Error.Private, Option[K]] =
...
//    }
//
//  def getKeyValue(key: K): IO[Core.Error.Private, Option[(K, V)]] =
...
//
//  def expiration(key: K): IO[Core.Error.Private, Option[Deadline]] =
//    map expiration Key.MapEntry(mapKey, key)
//
//  def timeLeft(key: K): IO[Core.Error.Private, Option[FiniteDuration]] =
//    expiration(key).map(_.map(_.timeLeft))
//
//  def closeDatabase(): IO[Core.Error.Private, Unit] =
//    baseMap().close()
//
