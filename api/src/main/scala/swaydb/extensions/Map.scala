...
//    */
//  def getValue(): IO[IO.Error, Option[V]] =
...
//    */
//  def get(key: K): IO[IO.Error, Option[V]] =
...
//    */
//  def getKey(key: K): IO[IO.Error, Option[K]] =
...
//    }
//
//  def getKeyValue(key: K): IO[IO.Error, Option[(K, V)]] =
...
//
//  def expiration(key: K): IO[IO.Error, Option[Deadline]] =
//    map expiration Key.MapEntry(mapKey, key)
//
//  def timeLeft(key: K): IO[IO.Error, Option[FiniteDuration]] =
//    expiration(key).map(_.map(_.timeLeft))
//
//  def closeDatabase(): IO[IO.Error, Unit] =
//    baseMap().close()
//
