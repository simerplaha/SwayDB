...
//    */
//  def getValue(): IO[swaydb.Error.Segment, Option[V]] =
...
//    */
//  def get(key: K): IO[swaydb.Error.Segment, Option[V]] =
...
//    */
//  def getKey(key: K): IO[swaydb.Error.Segment, Option[K]] =
...
//    }
//
//  def getKeyValue(key: K): IO[swaydb.Error.Segment, Option[(K, V)]] =
...
//
//  def expiration(key: K): IO[swaydb.Error.Segment, Option[Deadline]] =
//    map expiration Key.MapEntry(mapKey, key)
//
//  def timeLeft(key: K): IO[swaydb.Error.Segment, Option[FiniteDuration]] =
//    expiration(key).map(_.map(_.timeLeft))
//
//  def closeDatabase(): IO[swaydb.Error.Segment, Unit] =
//    baseMap().close()
//
