...
//    */
//  def getValue(): IO[Core.Error.Segment, Option[V]] =
...
//    */
//  def get(key: K): IO[Core.Error.Segment, Option[V]] =
...
//    */
//  def getKey(key: K): IO[Core.Error.Segment, Option[K]] =
...
//    }
//
//  def getKeyValue(key: K): IO[Core.Error.Segment, Option[(K, V)]] =
...
//
//  def expiration(key: K): IO[Core.Error.Segment, Option[Deadline]] =
//    map expiration Key.MapEntry(mapKey, key)
//
//  def timeLeft(key: K): IO[Core.Error.Segment, Option[FiniteDuration]] =
//    expiration(key).map(_.map(_.timeLeft))
//
//  def closeDatabase(): IO[Core.Error.Segment, Unit] =
//    baseMap().close()
//
