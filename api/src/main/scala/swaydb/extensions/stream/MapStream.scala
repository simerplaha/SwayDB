...
//                         isReverse: Boolean,
//                         mapsOnly: Boolean,
//                         thisMapKeyBytes: Slice[Byte])(implicit keySerializer: Serializer[K]): IO[swaydb.Error.Segment, Option[(Key[K], Option[V])]] =
...
//                                                                   optionValueSerializer: Serializer[Option[V]]) extends Streamed[(K, V), Tag.CoreIO] { self =>
//
...
//
//  private def headOptionInner: IO[swaydb.Error.Segment, Option[(Key[K], Option[V])]] = {
...
//  }
//
//  override def headOption: IO[swaydb.Error.Segment, Option[(K, V)]] =
...
//        })
//
//      override private[swaydb] def next(previous: (K, V)): IO[swaydb.Error.Segment, Option[(K, V)]] =
//        MapStream.step(
...
//    */
//  override def lastOption: IO[swaydb.Error.Segment, Option[(K, V)]] =
//    reverse.headOption
//
