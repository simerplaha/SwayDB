...
//                         isReverse: Boolean,
//                         mapsOnly: Boolean,
//                         thisMapKeyBytes: Slice[Byte])(implicit keySerializer: Serializer[K]): IO[Core.Error.Segment, Option[(Key[K], Option[V])]] =
...
//
//  private def headOptionInner: IO[Core.Error.Segment, Option[(Key[K], Option[V])]] = {
...
//  }
//
//  override def headOption: IO[Core.Error.Segment, Option[(K, V)]] =
...
//        })
//
//      override private[swaydb] def next(previous: (K, V)): IO[Core.Error.Segment, Option[(K, V)]] =
//        MapStream.step(
...
//    */
//  override def lastOption: IO[Core.Error.Segment, Option[(K, V)]] =
//    reverse.headOption
//
