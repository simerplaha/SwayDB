...
class NestedOptionValueSpec extends TestBaseEmbedded {
  ...
  "Option[Option[V]]" in {
    ...
    rootMap.stream.toSeq.get should contain only ((1, None))
    rootMap.keys.stream.toSeq.get should contain only 1
  ...
  "Option[Empty[V]]" in {
    ...
    rootMap.stream.toSeq.get should contain inOrderOnly((1, None), (2, Some(Value.NonEmpty("two"))), (3, None))
    rootMap.keys.stream.toSeq.get should contain inOrderOnly(1, 2, 3)
  }
