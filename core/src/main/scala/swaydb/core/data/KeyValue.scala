...
private[core] object KeyValue {
  ...
  object ReadOnly {
    ...
    sealed trait Fixed extends SegmentResponse {
      def toFromValue(): IO[Core.IO.Error, Value.FromValue]

      def toRangeValue(): IO[Core.IO.Error, Value.RangeValue]
    ...
    sealed trait Put extends KeyValue.ReadOnly.Fixed {
      ...
      def getOrFetchValue: IO[Core.IO.Error, Option[Slice[Byte]]]
      def time: Time
      def toFromValue(): IO[Core.IO.Error, Value.Put]
    ...
    sealed trait Remove extends KeyValue.ReadOnly.Fixed {
      ...
      def hasTimeLeftAtLeast(minus: FiniteDuration): Boolean
      def time: Time
      def toFromValue(): IO[Core.IO.Error, Value.Remove]
    ...
    sealed trait Update extends KeyValue.ReadOnly.Fixed {
      ...
      def time: Time
      def getOrFetchValue: IO[Core.IO.Error, Option[Slice[Byte]]]
      def toFromValue(): IO[Core.IO.Error, Value.Update]
    ...
    sealed trait Function extends KeyValue.ReadOnly.Fixed {
      def time: Time
      def getOrFetchFunction: IO[Core.IO.Error, Slice[Byte]]
      def toFromValue(): IO[Core.IO.Error, Value.Function]
      def copyWithTime(time: Time): Function
    }

    sealed trait PendingApply extends KeyValue.ReadOnly.Fixed {
      def getOrFetchApplies: IO[Core.IO.Error, Slice[Value.Apply]]
      def toFromValue(): IO[Core.IO.Error, Value.PendingApply]
      def time: Time
      def deadline: Option[Deadline]
    }
    ...
    sealed trait Range extends KeyValue.ReadOnly with SegmentResponse {
      ...
      def fetchFromValue: IO[Core.IO.Error, Option[Value.FromValue]]
      def fetchRangeValue: IO[Core.IO.Error, Value.RangeValue]
      def fetchFromAndRangeValue: IO[Core.IO.Error, (Option[Value.FromValue], Value.RangeValue)]
      def fetchFromOrElseRangeValue: IO[Core.IO.Error, Value.FromValue] =
...
private[swaydb] object Memory {
  ...
  case class Put(key: Slice[Byte],
    ...
    override def getOrFetchValue: IO[Core.IO.Error, Option[Slice[Byte]]] =
      IO.Success(value)

    override def toFromValue(): IO[Core.IO.Error, Value.Put] =
    ...
    override def toRangeValue(): IO[Core.IO.Error, Value.RangeValue] =
  ...
  case class Update(key: Slice[Byte],
    ...
    override def toFromValue(): IO[Core.IO.Error, Value.Update] =
    ...
    override def toRangeValue(): IO[Core.IO.Error, Value.Update] =
      toFromValue()
  }
  ...
  case class Function(key: Slice[Byte],
    ...
    override def getOrFetchFunction: IO[Core.IO.Error, Slice[Byte]] =
      IO.Success(function)

    override def toFromValue(): IO[Core.IO.Error, Value.Function] =
      IO.Success(Value.Function(function, time))

    override def copyWithTime(time: Time): Function =
      copy(time = time)

    override def toRangeValue(): IO[Core.IO.Error, Value.Function] =
      toFromValue()
  }
  ...
  case class PendingApply(key: Slice[Byte],
    ...
    override def getOrFetchApplies: IO[Core.IO.Error, Slice[Value.Apply]] =
      IO.Success(applies)

    override def toFromValue(): IO[Core.IO.Error, Value.PendingApply] =
      IO.Success(Value.PendingApply(applies))

    override def toRangeValue(): IO[Core.IO.Error, Value.PendingApply] =
      toFromValue()
  }
  ...
  case class Remove(key: Slice[Byte],
    ...
    override def toFromValue(): IO[Core.IO.Error, Value.Remove] =
      IO.Success(toRemoveValue())

    override def toRangeValue(): IO[Core.IO.Error, Value.Remove] =
      toFromValue()
  }
  ...
  case class Range(fromKey: Slice[Byte],
    ...
    override def fetchFromValue: IO[Core.IO.Error, Option[Value.FromValue]] =
      IO.Success(fromValue)

    override def fetchRangeValue: IO[Core.IO.Error, Value.RangeValue] =
      IO.Success(rangeValue)

    override def fetchFromAndRangeValue: IO[Core.IO.Error, (Option[Value.FromValue], Value.RangeValue)] =
...
private[core] object Transient {
  ...
  object Group {
    ...
    def apply(keyValues: Slice[Transient],
              ...
              binarySearchIndexConfig: BinarySearchIndexBlock.Config,
              hashIndexConfig: HashIndexBlock.Config,
              bloomFilterConfig: BloomFilterBlock.Config): IO[Core.IO.Error, Transient.Group] =
...
private[core] object Persistent {
  ...
  sealed trait SegmentResponse extends KeyValue.ReadOnly.SegmentResponse with Persistent {
    def toMemory(): IO[Core.IO.Error, Memory.SegmentResponse]

    def isValueCached: Boolean

    def toMemoryResponseOption(): IO[Core.IO.Error, Option[Memory.SegmentResponse]] =
      toMemory() map (Some(_))
  }
  ...
  case class Remove(private var _key: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.Remove] =
      IO.Success {
        Memory.Remove(
          key = key,
          deadline = deadline,
  ...
  case class Put(private var _key: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.Put] =
      getOrFetchValue map {
        value =>
          Memory.Put(
            key = key,
  ...
  case class Update(private var _key: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.Update] =
      getOrFetchValue map {
        value =>
          Memory.Update(
            key = key,
  ...
  case class Function(private var _key: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.Function] =
      getOrFetchFunction map {
        function =>
          Memory.Function(
            key = key,
  ...
  case class PendingApply(private var _key: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.PendingApply] =
  ...
  object Range {
    def apply(key: Slice[Byte],
              ...
              valueLength: Int,
              accessPosition: Int,
              isPrefixCompressed: Boolean): IO[Core.IO.Error, Persistent.Range] =
  ...
  case class Range(private var _fromKey: Slice[Byte],
    ...
    override def toMemory(): IO[Core.IO.Error, Memory.Range] =
      fetchFromAndRangeValue map {
        case (fromValue, rangeValue) =>
          Memory.Range(
            fromKey = fromKey,
  ...
  object Group {
    def apply(key: Slice[Byte],
              ...
              accessPosition: Int,
              deadline: Option[Deadline],
              isPrefixCompressed: Boolean): IO[Core.IO.Error, Group] =
