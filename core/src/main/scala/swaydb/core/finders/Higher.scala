...
object Higher {
  ...
  def apply(key: Slice[Byte],
    ...
    /**
      * Sets isNextLevelKnownEmpty to true if during an iteration a key returned None from next Level.
      * There is no need to repeatedly read for higher from next Level if one iteration returns empty.
      */
    @tailrec
    def higher(key: Slice[Byte], isNextLevelKnownEmpty: Boolean): Try[Option[KeyValue.ReadOnly.Put]] =
                      ...
                      if (Value.hasTimeLeft(rangeValue)) {
                        ...
                        case Success(Some(next)) =>
                            ...
                            FixedMerger(rangeValue.toMemory(next.key), next) match {
                              case Success(current) =>
                                ...
                                Min(current, None) match {
                                  ...
                                  case Success(None) =>
                                    higher(next.key, isNextLevelKnownEmpty = false) //current.key or next.key (either one! they both are the same here)
                          ...
                          //     20 ----to----> ∞
                            ...
                            get(current.toKey) match {
                              ...
                              case Success(_) =>
                                higher(current.toKey, isNextLevelKnownEmpty = isNextLevelKnownEmpty)
                        ...
                        case Success(None) => //if there is no underlying key in lower Levels, jump to the next highest key (toKey).
                          get(current.toKey) match {
                            ...
                            case Success(_) =>
                              higher(current.toKey, isNextLevelKnownEmpty = true)
                        ...
                        get(current.toKey) match {
                          ...
                          case Success(_) =>
                            higher(current.toKey, isNextLevelKnownEmpty = isNextLevelKnownEmpty) //don't know! as it's not fetched.
                                  ...
                                  FixedMerger(rangeValue.toMemory(next.key), next) match {
                                    case Success(currentValue) =>
                                      Min(currentValue, None) match { //return applied value with next key-value as the current value.
                                        ...
                                        case Success(None) =>
                                          higher(current.key, isNextLevelKnownEmpty = false)
                              ...
                              FixedMerger(rangeValue.toMemory(next.key), next) match {
                                case Success(current) =>
                                  Min(current, None) match { //return applied value with next key-value as the current value.
                                    ...
                                    case Success(None) =>
                                      higher(next.key, isNextLevelKnownEmpty = false)
                                  ...
                                  Min(fromValue.toMemory(current.fromKey), None) match { //return applied value with next key-value as the current value.
                                    ...
                                    case Success(None) =>
                                      get(current.toKey) match {
                                        ...
                                        case Success(_) =>
                                          higher(current.toKey, isNextLevelKnownEmpty = false)
    ...
    higher(key, isNextLevelKnownEmpty = false)
  }
}
