...
private[file] object MMAPFile {
  ...
  def read(path: Path): IO[Core.Error, MMAPFile] =
  ...
  def write(path: Path,
            bufferSize: Long): IO[Core.Error, MMAPFile] =
...
private[file] class MMAPFile(val path: Path,
  ...
  /**
    ...
    * [[Core.Error.NullMappedByteBuffer]] so this request will get retried.
    *
    * [[NullPointerException]] should not leak outside.
    *
    * FIXME - Switch to using Option.
    */
  def recoverFromNullPointer[T](f: => T): IO[Core.Error, T] =
    IO[Core.Error, T](f) recoverWith {
  ...
  private def extendBuffer(bufferSize: Long): IO[Core.Error, Unit] =
    recoverFromNullPointer {
  ...
  @tailrec
  final def append(slice: Slice[Byte]): IO[Core.Error, Unit] =
    recoverFromNullPointer[Unit](buffer.put(slice.toByteBufferWrap)) match {
      ...
      case IO.Failure(Core.Error.Fatal(ex: BufferOverflowException)) =>
