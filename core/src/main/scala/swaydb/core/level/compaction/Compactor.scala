...
private[core] object Compactor extends CompactionStrategy[CompactorState] with LazyLogging {
  ...
  def createActor(levels: List[LevelRef],
    ...
    if (levels.size != executionContexts.size)
      IO.Failure(swaydb.Error.Unknown(new IllegalStateException(s"Number of ExecutionContexts(${executionContexts.size}) is not the same as number of Levels(${levels.size}).")))
    else
      levels
          ...
          case (jobs, (level, CompactionExecutionContext.Shared)) => //share with previous thread pool.
            jobs.lastOption match {
              ...
              case None =>
                ...
                IO.Failure(swaydb.Error.Unknown(new IllegalStateException("Shared ExecutionContext submitted without Create.")))
            }
        }
        .map {
          jobs =>
  ...
  def createCompactor(zero: LevelZero,
    ...
    } getOrElse IO.Failure(swaydb.Error.Unknown(new Exception("Compaction not started because there is no lower level.")))
