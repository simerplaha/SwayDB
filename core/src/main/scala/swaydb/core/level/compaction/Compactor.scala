...
private[core] object Compactor extends CompactionStrategy[CompactorState] with LazyLogging {
  ...
  def createActor(levels: List[LevelRef],
                  executionContexts: List[CompactionExecutionContext])(implicit ordering: CompactionOrdering): IO[Core.Error, WiredActor[CompactionStrategy[CompactorState], CompactorState]] =
  ...
  def createCompactor(zero: LevelZero,
                      executionContexts: List[CompactionExecutionContext])(implicit compactionOrdering: CompactionOrdering): IO[Core.Error, WiredActor[CompactionStrategy[CompactorState], CompactorState]] =
    zero.nextLevel map {
      nextLevel =>
  ...
  def createAndListen(zero: LevelZero,
                      ...
                      copyForwardAllOnStart: Boolean)(implicit compactionOrdering: CompactionOrdering): IO[Core.Error, WiredActor[CompactionStrategy[CompactorState], CompactorState]] =
    createCompactor(
      zero = zero,
      executionContexts = executionContexts
    ) map {
