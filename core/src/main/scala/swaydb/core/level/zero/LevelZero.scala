...
private[core] object LevelZero extends LazyLogging {
  ...
  def apply(mapSize: Long,
            ...
            throttle: LevelZeroMeter => FiniteDuration)(implicit keyOrder: KeyOrder[Slice[Byte]],
                                                        ...
                                                        limiter: FileLimiter,
                                                        functionStore: FunctionStore): IO[Core.Error, LevelZero] = {
  ...
  def delete(zero: LevelZero): IO[Core.Error, Unit] =
    zero
      .close
      .flatMap {
        _ =>
...
private[core] case class LevelZero(path: Path,
  ...
  def remove(fromKey: Slice[Byte], toKey: Slice[Byte], at: Deadline): IO[Core.Error, IO.OK] =
    assertKey(fromKey) {
      assertKey(toKey) {
        if (fromKey equiv toKey)
          remove(fromKey)
  ...
  def update(key: Slice[Byte], value: Slice[Byte]): IO[Core.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Update(key, Some(value), None, timer.next)))
    }

  def update(key: Slice[Byte], value: Option[Slice[Byte]]): IO[Core.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Update(key, value, None, timer.next)))
    }

  def update(fromKey: Slice[Byte], toKey: Slice[Byte], value: Slice[Byte]): IO[Core.Error, IO.OK] =
    update(fromKey, toKey, Some(value))

  def update(fromKey: Slice[Byte], toKey: Slice[Byte], value: Option[Slice[Byte]]): IO[Core.Error, IO.OK] =
    assertKey(fromKey) {
      assertKey(toKey) {
        if (fromKey equiv toKey)
  ...
  def clear(): IO.Defer[Core.Error, IO.OK] =
    headKey flatMap {
      case Some(headKey) =>
        lastKey flatMap {
          case Some(lastKey) =>
  ...
  def applyFunction(key: Slice[Byte], function: Slice[Byte]): IO[Core.Error, IO.OK] =
  ...
  def applyFunction(fromKey: Slice[Byte], toKey: Slice[Byte], function: Slice[Byte]): IO[Core.Error, IO.OK] =
    if (!functionStore.exists(function))
      IO.failed(new Exception("Function does not exists in function store."))
    else
      assertKey(fromKey) {
  ...
  private def getFromNextLevel(key: Slice[Byte],
                               mapsIterator: util.Iterator[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  def get(key: Slice[Byte]): IO.Defer[Core.Error, Option[ReadOnly.Put]] =
    find(
      key = key,
      currentMap = maps.map,
  ...
  def getKey(key: Slice[Byte]): IO.Defer[Core.Error, Option[Slice[Byte]]] =
  ...
  def last: IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
    nextLevel map {
      nextLevel =>
  ...
  def ceiling(key: Slice[Byte],
              currentMap: map.Map[Slice[Byte], Memory.SegmentResponse],
              otherMaps: List[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  def floor(key: Slice[Byte]): IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  /**
    ...
    */
  def higher(key: Slice[Byte]): IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
    findHigher(
      key = key,
      currentMap = maps.map,
  ...
  /**
    ...
    */
  def lower(key: Slice[Byte]): IO.Defer[Core.Error, Option[KeyValue.ReadOnly.Put]] =
    findLower(
      key = key,
      currentMap = maps.map,
      otherMaps = maps.queuedMaps.toList
  ...
  def contains(key: Slice[Byte]): IO.Defer[Core.Error, Boolean] =
    get(key).mapDeferred(_.isDefined)

  def valueSize(key: Slice[Byte]): IO.Defer[Core.Error, Option[Int]] =
    get(key) mapDeferred {
      result =>
        result map {
          response =>
  ...
  def bloomFilterKeyValueCount: IO[Core.Error, Int] = {
  ...
  def deadline(key: Slice[Byte]): IO.Defer[Core.Error, Option[Deadline]] =
    get(key) mapDeferred {
      result =>
        result flatMap {
          response =>
  ...
  def mightContainFunction(functionId: Slice[Byte]): IO[Core.Error, Boolean] =
