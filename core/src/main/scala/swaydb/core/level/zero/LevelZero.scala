...
private[core] object LevelZero extends LazyLogging {
  ...
  def apply(mapSize: Long,
            ...
            throttle: LevelZeroMeter => FiniteDuration)(implicit keyOrder: KeyOrder[Slice[Byte]],
                                                        ...
                                                        limiter: FileLimiter,
                                                        functionStore: FunctionStore): IO[Core.Error.Private, LevelZero] = {
  ...
  def delete(zero: LevelZero): IO[Core.Error.Private, Unit] =
    zero
      .close
      .flatMap {
        _ =>
...
private[core] case class LevelZero(path: Path,
  ...
  private def getFromNextLevel(key: Slice[Byte],
                               mapsIterator: util.Iterator[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.Error.Private, Option[KeyValue.ReadOnly.Put]] =
  ...
  def get(key: Slice[Byte]): IO.Defer[Core.Error.Private, Option[ReadOnly.Put]] =
    find(
      key = key,
      currentMap = maps.map,
  ...
  def getKey(key: Slice[Byte]): IO.Defer[Core.Error.Private, Option[Slice[Byte]]] =
  ...
  def lastKey: IO.Defer[Core.Error.Private, Option[Slice[Byte]]] =
  ...
  def head: IO.Defer[Core.Error.Private, Option[KeyValue.ReadOnly.Put]] =
    nextLevel map {
      nextLevel =>
  ...
  def last: IO.Defer[Core.Error.Private, Option[KeyValue.ReadOnly.Put]] =
    nextLevel map {
      nextLevel =>
  ...
  def ceiling(key: Slice[Byte],
              currentMap: map.Map[Slice[Byte], Memory.SegmentResponse],
              otherMaps: List[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.Error.Private, Option[KeyValue.ReadOnly.Put]] =
  ...
  def floor(key: Slice[Byte]): IO.Defer[Core.Error.Private, Option[KeyValue.ReadOnly.Put]] =
  ...
  def contains(key: Slice[Byte]): IO.Defer[Core.Error.Private, Boolean] =
    get(key).mapDeferred(_.isDefined)

  def valueSize(key: Slice[Byte]): IO.Defer[Core.Error.Private, Option[Int]] =
    get(key) mapDeferred {
      result =>
        result map {
          response =>
  ...
  def bloomFilterKeyValueCount: IO[Core.Error.Private, Int] = {
  ...
  def deadline(key: Slice[Byte]): IO.Defer[Core.Error.Private, Option[Deadline]] =
    get(key) mapDeferred {
      result =>
        result flatMap {
          response =>
  ...
  def mightContainFunction(functionId: Slice[Byte]): IO[Core.Error.Private, Boolean] =
