...
private[core] object LevelZero extends LazyLogging {
  ...
  def apply(mapSize: Long,
            ...
            throttle: LevelZeroMeter => FiniteDuration)(implicit keyOrder: KeyOrder[Slice[Byte]],
                                                        ...
                                                        limiter: FileLimiter,
                                                        functionStore: FunctionStore): IO[Core.IO.Error, LevelZero] = {
  ...
  def delete(zero: LevelZero): IO[Core.IO.Error, Unit] =
    zero
      .close
      .flatMap {
        _ =>
...
private[core] case class LevelZero(path: Path,
  ...
  def assertKey(key: Slice[Byte])(block: => IO[Core.IO.Error, IO.OK]): IO[Core.IO.Error, IO.OK] =
    if (key.isEmpty)
      IO.failed(new IllegalArgumentException("Input key(s) cannot be empty."))
    else
      block

  def put(key: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put[Slice[Byte], Memory.SegmentResponse](key, Memory.Put(key, None, None, timer.next)))
    }

  def put(key: Slice[Byte], value: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Put(key, Some(value), None, timer.next)))
    }

  def put(key: Slice[Byte], value: Option[Slice[Byte]], removeAt: Deadline): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Put(key, value, Some(removeAt), timer.next)))
    }

  def put(key: Slice[Byte], value: Option[Slice[Byte]]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Put(key, value, None, timer.next)))
    }

  def put(entry: Timer => MapEntry[Slice[Byte], Memory.SegmentResponse]): IO[Core.IO.Error, IO.OK] =
    maps write entry

  def remove(key: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put[Slice[Byte], Memory.Remove](key, Memory.Remove(key, None, timer.next)))
    }

  def remove(key: Slice[Byte], at: Deadline): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put[Slice[Byte], Memory.Remove](key, Memory.Remove(key, Some(at), timer.next)))
    }

  def remove(fromKey: Slice[Byte], toKey: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    assertKey(fromKey) {
      assertKey(toKey) {
  ...
  def remove(fromKey: Slice[Byte], toKey: Slice[Byte], at: Deadline): IO[Core.IO.Error, IO.OK] =
    assertKey(fromKey) {
      assertKey(toKey) {
        if (fromKey equiv toKey)
          remove(fromKey)
  ...
  def update(key: Slice[Byte], value: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Update(key, Some(value), None, timer.next)))
    }

  def update(key: Slice[Byte], value: Option[Slice[Byte]]): IO[Core.IO.Error, IO.OK] =
    assertKey(key) {
      maps.write(timer => MapEntry.Put(key, Memory.Update(key, value, None, timer.next)))
    }

  def update(fromKey: Slice[Byte], toKey: Slice[Byte], value: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    update(fromKey, toKey, Some(value))

  def update(fromKey: Slice[Byte], toKey: Slice[Byte], value: Option[Slice[Byte]]): IO[Core.IO.Error, IO.OK] =
    assertKey(fromKey) {
      assertKey(toKey) {
        if (fromKey equiv toKey)
  ...
  def clear(): IO.Defer[Core.IO.Error, IO.OK] =
    headKey flatMap {
      case Some(headKey) =>
        lastKey flatMap {
          case Some(lastKey) =>
  ...
  def applyFunction(key: Slice[Byte], function: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
  ...
  def applyFunction(fromKey: Slice[Byte], toKey: Slice[Byte], function: Slice[Byte]): IO[Core.IO.Error, IO.OK] =
    if (!functionStore.exists(function))
      IO.failed(new Exception("Function does not exists in function store."))
    else
      assertKey(fromKey) {
  ...
  private def getFromNextLevel(key: Slice[Byte],
                               mapsIterator: util.Iterator[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.IO.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  def get(key: Slice[Byte]): IO.Defer[Core.IO.Error, Option[ReadOnly.Put]] =
    find(
      key = key,
      currentMap = maps.map,
  ...
  def getKey(key: Slice[Byte]): IO.Defer[Core.IO.Error, Option[Slice[Byte]]] =
  ...
  def last: IO.Defer[Core.IO.Error, Option[KeyValue.ReadOnly.Put]] =
    nextLevel map {
      nextLevel =>
  ...
  def ceiling(key: Slice[Byte],
              currentMap: map.Map[Slice[Byte], Memory.SegmentResponse],
              otherMaps: List[map.Map[Slice[Byte], Memory.SegmentResponse]]): IO.Defer[Core.IO.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  def floor(key: Slice[Byte]): IO.Defer[Core.IO.Error, Option[KeyValue.ReadOnly.Put]] =
  ...
  def contains(key: Slice[Byte]): IO.Defer[Core.IO.Error, Boolean] =
    get(key).mapDeferred(_.isDefined)

  def valueSize(key: Slice[Byte]): IO.Defer[Core.IO.Error, Option[Int]] =
    get(key) mapDeferred {
      result =>
        result map {
          response =>
  ...
  def bloomFilterKeyValueCount: IO[Core.IO.Error, Int] = {
  ...
  def deadline(key: Slice[Byte]): IO.Defer[Core.IO.Error, Option[Deadline]] =
    get(key) mapDeferred {
      result =>
        result flatMap {
          response =>
  ...
  def mightContainKey(key: Slice[Byte]): IO[Core.IO.Error, Boolean] =
  ...
  def mightContainFunction(functionId: Slice[Byte]): IO[Core.IO.Error, Boolean] =
