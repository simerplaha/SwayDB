...
object LevelZeroMapEntryReader {
  ...
  implicit object Level0RemoveReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.Remove]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.Remove]]] =
      for {
  ...
  implicit object Level0PutReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.Put]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.Put]]] =
      for {
        keyLength <- reader.readInt()
        key <- reader.read(keyLength).map(_.unslice())
        timeLength <- reader.readInt()
  ...
  implicit object Level0UpdateReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.Update]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.Update]]] =
      for {
        keyLength <- reader.readInt()
        key <- reader.read(keyLength).map(_.unslice())
        timeLength <- reader.readInt()
  ...
  implicit object Level0FunctionReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.Function]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.Function]]] =
      for {
        keyLength <- reader.readInt()
        key <- reader.read(keyLength).map(_.unslice())
        timeLength <- reader.readInt()
  ...
  implicit object Level0RangeReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.Range]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.Range]]] =
      for {
  ...
  implicit object Level0PendingApplyReader extends MapEntryReader[MapEntry.Put[Slice[Byte], Memory.PendingApply]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry.Put[Slice[Byte], Memory.PendingApply]]] =
      for {
        keyLength <- reader.readInt()
        key <- reader.read(keyLength).map(_.unslice())
        valueLength <- reader.readInt()
  ...
  implicit object Level0Reader extends MapEntryReader[MapEntry[Slice[Byte], Memory.SegmentResponse]] {
    ...
    override def read(reader: Reader[Core.Error.Private]): IO[Core.Error.Private, Option[MapEntry[Slice[Byte], Memory.SegmentResponse]]] =
