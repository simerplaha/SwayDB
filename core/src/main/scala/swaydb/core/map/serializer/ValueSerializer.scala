...
sealed trait ValueSerializer[T] {
  ...
  def read(reader: Reader[Core.Error]): IO[Core.Error, T]

  def read(bytes: Slice[Byte]): IO[Core.Error, T] =
    read(Reader(bytes))
...
object ValueSerializer {
  ...
  def readDeadline(reader: Reader[Core.Error]): IO[Core.Error, Option[Deadline]] =
  ...
  def readTime(reader: Reader[Core.Error]): IO[Core.Error, Time] =
  ...
  def readRemainingTime(reader: Reader[Core.Error]): IO[Core.Error, Time] =
  ...
  def readValue(reader: Reader[Core.Error]): IO[Core.Error, Option[Slice[Byte]]] =
    reader.readRemaining() map {
      remaining =>
        if (remaining.isEmpty)
          None
  ...
  implicit object ValuePutSerializer extends ValueSerializer[Value.Put] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Value.Put] =
      for {
  ...
  implicit object ValueUpdateSerializer extends ValueSerializer[Value.Update] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Value.Update] =
      for {
        deadline <- readDeadline(reader)
        time <- readTime(reader)
        value <- readValue(reader)
  ...
  implicit object ValueRemoveSerializer extends ValueSerializer[Value.Remove] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Value.Remove] =
      for {
        deadline <- readDeadline(reader)
  ...
  implicit object ValueFunctionSerializer extends ValueSerializer[Value.Function] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Value.Function] =
  ...
  implicit object ValueSliceApplySerializer extends ValueSerializer[Slice[Value.Apply]] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Slice[Value.Apply]] =
      reader.readIntUnsigned() flatMap {
        count =>
  ...
  implicit object ValuePendingApplySerializer extends ValueSerializer[Value.PendingApply] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Value.PendingApply] =
  ...
  implicit object SeqOfBytesSerializer extends ValueSerializer[Seq[Slice[Byte]]] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, Seq[Slice[Byte]]] =
  ...
  implicit object TupleOfBytesSerializer extends ValueSerializer[(Slice[Byte], Slice[Byte])] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, (Slice[Byte], Slice[Byte])] =
  ...
  implicit object TupleBytesAndOptionBytesSerializer extends ValueSerializer[(Slice[Byte], Option[Slice[Byte]])] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, (Slice[Byte], Option[Slice[Byte]])] =
      reader.readIntUnsigned() flatMap {
        id =>
          if (id == 0)
            reader.readRemaining() map {
  ...
  implicit object IntMapListBufferSerializer extends ValueSerializer[mutable.Map[Int, Iterable[(Slice[Byte], Slice[Byte])]]] {
    ...
    override def read(reader: Reader[Core.Error]): IO[Core.Error, mutable.Map[Int, Iterable[(Slice[Byte], Slice[Byte])]]] =
      reader.get() flatMap {
        format =>
          if (format != formatId)
            IO.Failure(Core.Error.Fatal(new Exception(s"Invalid formatID: $format")))
          else
  ...
  def read[T](value: Slice[Byte])(implicit serializer: ValueSerializer[T]): IO[Core.Error, T] =
    serializer.read(value)

  def read[T](reader: Reader[Core.Error])(implicit serializer: ValueSerializer[T]): IO[Core.Error, T] =
