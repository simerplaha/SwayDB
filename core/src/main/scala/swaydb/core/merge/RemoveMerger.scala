...
private[core] object RemoveMerger {
  ...
  def apply(newKeyValue: ReadOnly.Remove,
            oldKeyValue: ReadOnly.Function)(implicit timeOrder: TimeOrder[Slice[Byte]]): IO[Core.Error, ReadOnly.Fixed] =
    if (newKeyValue.time > oldKeyValue.time)
      newKeyValue.deadline match {
        case None =>
  ...
  def apply(newKeyValue: ReadOnly.Remove,
            oldKeyValue: Value.Apply)(implicit timeOrder: TimeOrder[Slice[Byte]]): IO[Core.Error, ReadOnly.Fixed] =
    if (newKeyValue.time > oldKeyValue.time)
      oldKeyValue match {
