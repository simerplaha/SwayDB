...
private[core] object Block extends LazyLogging {
  ...
  case class Header[O](compressionInfo: Option[CompressionInfo],
                       headerReader: Reader[Core.Error.Segment],
                       headerSize: Int,
                       offset: O)
  ...
  def block(headerSize: Int,
            ...
            blockName: String): IO[Core.Error.Segment, Slice[Byte]] =
  ...
  def block(openSegment: SegmentBlock.Open,
            compressions: Seq[CompressionInternal],
            blockName: String): IO[Core.Error.Segment, SegmentBlock.Closed] =
    if (compressions.isEmpty) {
  ...
  private def readCompressionInfo(formatID: Int,
                                  headerSize: Int,
                                  reader: Reader[Core.Error.Segment]): IO[Core.Error.Segment, Option[CompressionInfo]] =
  ...
  def readHeader[O <: BlockOffset](reader: BlockRefReader[O])(implicit blockOps: BlockOps[O, _]): IO[Core.Error.Segment, Block.Header[O]] = {
    for {
  ...
  def unblock[O <: BlockOffset, B <: Block[O]](bytes: Slice[Byte])(implicit blockOps: BlockOps[O, B]): IO[Core.Error.Segment, UnblockedReader[O, B]] =
    unblock(BlockRefReader(bytes))

  def unblock[O <: BlockOffset, B <: Block[O]](ref: BlockRefReader[O])(implicit blockOps: BlockOps[O, B]): IO[Core.Error.Segment, UnblockedReader[O, B]] =
