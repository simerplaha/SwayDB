...
private[core] object HashIndexBlock extends LazyLogging {
  ...
  def read(header: Block.Header[HashIndexBlock.Offset]): IO[Core.Error.Private, HashIndexBlock] =
    for {
  ...
  def write(key: Slice[Byte],
            value: Int,
            state: State): IO[Core.Error.Private, Boolean] = {
  ...
  private[block] def search[R](key: Slice[Byte],
                               ...
                               assertValue: Int => IO[Core.Error.Private, Option[R]]): IO[Core.Error.Private, Option[R]] = {
    ...
    @tailrec
    def doFind(probe: Int, checkedHashIndexes: mutable.HashSet[Int]): IO[Core.Error.Private, Option[R]] =
            ...
            case IO.Success(possibleValueBytes) =>
              ...
              if (possibleValueBytes.isEmpty || possibleValueBytes.head != 0) {
                ...
                possibleValueWithoutHeader.readIntUnsigned() match {
                  ...
                  case IO.Failure(error) =>
                    IO.Failure[Core.Error.Private, Option[R]](error = error)
                }
              }
  ...
  def search(key: Slice[Byte],
             ...
             valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]])(implicit keyOrder: KeyOrder[Slice[Byte]]): IO[Core.Error.Private, Option[Persistent]] = {
    val matcher =
  ...
  implicit object HashIndexBlockOps extends BlockOps[HashIndexBlock.Offset, HashIndexBlock] {
    ...
    override def readBlock(header: Block.Header[Offset]): IO[Core.Error.Private, HashIndexBlock] =
