...
object SegmentFooterBlock {
  ...
  def writeAndClose(state: State, closedBlocks: ClosedBlocks): IO[Core.Error.Private, State] =
    IO {
  ...
  def read(reader: UnblockedReader[SegmentBlock.Offset, SegmentBlock]): IO[Core.Error.Private, SegmentFooterBlock] =
    try {
  ...
  implicit object SegmentFooterBlockUpdated extends BlockOps[SegmentFooterBlock.Offset, SegmentFooterBlock] {
    ...
    override def readBlock(header: Block.Header[Offset]): IO[Core.Error.Private, SegmentFooterBlock] =
