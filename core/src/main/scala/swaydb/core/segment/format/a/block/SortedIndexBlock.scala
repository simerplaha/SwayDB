...
private[core] object SortedIndexBlock extends LazyLogging {
  ...
  implicit object SortedIndexBlockOps extends BlockOps[SortedIndexBlock.Offset, SortedIndexBlock] {
    ...
    override def readBlock(header: Block.Header[Offset]): IO[IO.Error, SortedIndexBlock] =
  ...
  def write(keyValue: Transient, state: SortedIndexBlock.State): IO[IO.Error, Unit] =
    IO {
  ...
  def read(header: Block.Header[SortedIndexBlock.Offset]): IO[IO.Error, SortedIndexBlock] =
    for {
  ...
  private def readNextKeyValue(previous: Persistent,
                               ...
                               valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]]): IO[IO.Error, Persistent] =
    readNextKeyValue(
  ...
  def readAll(keyValueCount: Int,
              ...
              valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]],
              addTo: Option[Slice[KeyValue.ReadOnly]] = None): IO[IO.Error, Slice[KeyValue.ReadOnly]] =
    try {
  ...
  def search(key: Slice[Byte],
             ...
             indexReader: UnblockedReader[SortedIndexBlock.Offset, SortedIndexBlock],
             valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]])(implicit order: KeyOrder[Slice[Byte]]): IO[IO.Error, Option[Persistent]] =
  ...
  private def search(matcher: KeyMatcher,
                     startFrom: Option[Persistent],
                     indexReader: UnblockedReader[SortedIndexBlock.Offset, SortedIndexBlock],
                     valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]]): IO[IO.Error, Option[Persistent]] =
    startFrom match {
      case Some(startFrom) =>
  ...
  def findAndMatchOrNextMatch(matcher: KeyMatcher,
                              ...
                              valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]]): IO[IO.Error, KeyMatcher.Result] =
    readNextKeyValue(
      fromPosition = fromOffset,
  ...
  def matchOrNext(previous: Persistent,
                  ...
                  matcher: KeyMatcher,
                  indexReader: UnblockedReader[SortedIndexBlock.Offset, SortedIndexBlock],
                  valuesReader: Option[UnblockedReader[ValuesBlock.Offset, ValuesBlock]]): IO[IO.Error, KeyMatcher.Result.Complete] =
    matcher(
      previous = previous,
      next = next,
