...
private[core] object UnblockedReader {
  ...
  def asUnblocked[O <: BlockOffset, B <: Block[O]](blockedReader: BlockedReader[O, B])(implicit blockOps: BlockOps[O, B]): IO[Core.Error.Private, UnblockedReader[O, B]] =
...
private[core] class UnblockedReader[O <: BlockOffset, B <: Block[O]] private(val block: B,
                                                                             private[reader] val reader: Reader[Core.Error.Private]) extends BlockReader with LazyLogging {
  ...
  def readAllAndGetReader()(implicit blockOps: BlockOps[O, B]): IO[Core.Error.Private, UnblockedReader[O, B]] =
    readAll()
      .map {
        bytes =>
          UnblockedReader[O, B](
