...
private[core] object UnblockedReader {
  ...
  def asUnblocked[O <: BlockOffset, B <: Block[O]](blockedReader: BlockedReader[O, B])(implicit blockOps: BlockOps[O, B]): IO[Core.Error, UnblockedReader[O, B]] =
...
private[core] class UnblockedReader[O <: BlockOffset, B <: Block[O]] private(val block: B,
  ...
  def readAllAndGetReader()(implicit blockOps: BlockOps[O, B]): IO[Core.Error, UnblockedReader[O, B]] =
    readAll()
      .map {
        bytes =>
          UnblockedReader[O, B](
