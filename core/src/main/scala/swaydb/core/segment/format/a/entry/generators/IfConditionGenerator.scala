...
object IfConditionGenerator extends App {
  ...
  def generateBinarySearchConditions(ids: List[BaseEntryId]): String = {
    if (ids.size == 1) {
      ...
      val targetFunction =
        s"Some(reader(${typedId.name}, keySize, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      ...
      val targetFunction1 =
        s"Some(reader(${typedId1.name}, keySize, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"

      val targetFunction2 =
        s"Some(reader(${typedId2.name}, keySize, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      ...
      s"if(baseId == ${mid.baseId})" + {
        s"\nSome(reader(${mid.name}, keySize, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      } + {
