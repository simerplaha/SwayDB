...
object IfConditionGenerator extends App {
  ...
  def generateBinarySearchConditions(ids: List[BaseEntryId]): String = {
    if (ids.size == 1) {
      ...
      val targetFunction =
        s"reader(${typedId.name}, keyValueId, sortedIndexEndOffset, sortedIndexAccessPosition, headerKeyBytes, indexReader, valuesReader, indexOffset, previous)"
      ...
      val targetFunction1 =
        s"reader(${typedId1.name}, keyValueId, sortedIndexEndOffset, sortedIndexAccessPosition, headerKeyBytes, indexReader, valuesReader, indexOffset, previous)"

      val targetFunction2 =
        s"reader(${typedId2.name}, keyValueId, sortedIndexEndOffset, sortedIndexAccessPosition, headerKeyBytes, indexReader, valuesReader, indexOffset, previous)"
      ...
      s"if(baseId == ${mid.baseId})" + {
        s"\nreader(${mid.name}, keyValueId, sortedIndexEndOffset, sortedIndexAccessPosition, headerKeyBytes, indexReader, valuesReader, indexOffset, previous)"
      } + {
