...
object IfConditionGenerator extends App {
  ...
  def generateBinarySearchConditions(ids: List[BaseEntryId]): String = {
    if (ids.size == 1) {
      ...
      val targetFunction =
        s"Some(reader(${typedId.name}, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      ...
      val targetFunction1 =
        s"Some(reader(${typedId1.name}, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"

      val targetFunction2 =
        s"Some(reader(${typedId2.name}, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      ...
      s"if(baseId == ${mid.baseId})" + {
        s"\nSome(reader(${mid.name}, keyValueId, accessPosition, keyInfo, indexReader, valueCache, indexOffset, nextIndexOffset, nextIndexSize, previous))"
      } + {
