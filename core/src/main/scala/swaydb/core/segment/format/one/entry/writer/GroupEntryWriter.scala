...
import swaydb.core.segment.format.one.entry.id.{GroupKeyFullyCompressedEntryId, GroupKeyPartiallyCompressedEntryId, GroupKeyUncompressedEntryId}
import swaydb.core.util.Bytes._
import swaydb.data.slice.Slice

object GroupEntryWriter {

  /**
    * @return indexEntry, valueBytes, valueOffsetBytes, nextValuesOffsetPosition
    */
  def write(current: Transient.Group,
          ...
          case (_, remainingBytes) if remainingBytes.isEmpty =>
            val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
              ValueWriter.write(
                ...
                id = GroupKeyFullyCompressedEntryId.KeyFullyCompressed,
...
//            valueBytes foreach (valueBytes => assert(valueBytes.isFull, s"valueBytes is not full actual: ${valueBytes.written} - expected: ${valueBytes.size}"))
          ...
          case (commonBytes, remainingBytes) =>
            val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
              ValueWriter.write(
                current = current,
                compressDuplicateValues = compressDuplicateValues,
                id = GroupKeyPartiallyCompressedEntryId.KeyPartiallyCompressed,
      ...
      val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
        ValueWriter.write(
          current = current,
          compressDuplicateValues = compressDuplicateValues,
          id = GroupKeyUncompressedEntryId.KeyUncompressed,
