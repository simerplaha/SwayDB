                    ...
                    _ =>
                      ...
                      val indexEntry =
                        DeadlineWriter.write(
                          current = current,
                          getDeadlineId = id.valueFullyCompressed,
                          plusSize = plusSize
                        )
                ...
                compress(Slice.writeInt(previous.currentStartValueOffsetPosition), currentValueOffsetBytes, 1) map {
                  case (valueOffsetCommonBytes, valueOffsetRemainingBytes) =>
                    val valueOffsetId =
                      if (valueOffsetCommonBytes == 1)
                        id.valueUncompressed.valueOffsetOneCompressed
                      else if (valueOffsetCommonBytes == 2)
                        id.valueUncompressed.valueOffsetTwoCompressed
                      else if (valueOffsetCommonBytes == 3)
                        id.valueUncompressed.valueOffsetThreeCompressed
                      else
                  ...
                  compress(Slice.writeInt(previousValue.size), Slice.writeInt(currentValue.size), 1) map {
                    case (valueLengthCommonBytes, valueLengthRemainingBytes) =>
                      val valueLengthId =
                        if (valueLengthCommonBytes == 1)
                          id.valueUncompressed.valueOffsetUncompressed.valueLengthOneCompressed
                        else if (valueLengthCommonBytes == 2)
                          id.valueUncompressed.valueOffsetUncompressed.valueLengthTwoCompressed
                        else if (valueLengthCommonBytes == 3)
                          id.valueUncompressed.valueOffsetUncompressed.valueLengthThreeCompressed
                        else if (valueLengthCommonBytes == 4)
                          id.valueUncompressed.valueOffsetUncompressed.valueLengthFullyCompressed
                        else
                          throw new Exception(s"Fatal exception: valueLengthCommonBytes = $valueLengthCommonBytes")
                    ...
                    val indexEntryBytes =
                      DeadlineWriter.write(
                        current = current,
                        getDeadlineId = id.valueUncompressed.valueOffsetUncompressed.valueLengthUncompressed,
