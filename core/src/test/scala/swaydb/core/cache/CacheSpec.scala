...
class CacheSpec extends WordSpec with Matchers with MockFactory {
  ...
  def getTestCache(isBlockIO: Boolean,
                   ...
                   isReserved: Boolean,
                   stored: Boolean,
                   initialValue: Option[Int]): (Unit => IO[swaydb.Error.Segment, Int]) => Cache[swaydb.Error.Segment, Unit, Int] =
    if (isBlockIO)
      Cache.deferredIO(getIOStrategy(isConcurrent, isSynchronised, isReserved, stored), swaydb.Error.ReservedResource(Reserve(name = "test")))
      ...
      Cache.reservedIO[swaydb.Error.Segment, swaydb.Error.ReservedResource, Unit, Int](stored = stored, swaydb.Error.ReservedResource(Reserve(name = "test")), initial = initialValue)
    else
      Cache.io(getIOStrategy(isConcurrent, isSynchronised, isReserved, stored)(()), swaydb.Error.ReservedResource(Reserve(name = "test")), initialValue) //then it's strategy IO
  ...
  "it" should {
    "fetch data only once on success" in {
      def doTest(isBlockIO: Boolean, isConcurrent: Boolean, isSynchronised: Boolean, isReserved: Boolean) = {
        ...
        val cache = getTestCache(isBlockIO, isConcurrent, isSynchronised, isReserved, stored = true, initialValue = initialValue)(_ => mock.apply())
    ...
    "store cache value on mapStored" in {
      def doTest(isBlockIO: Boolean, isConcurrent: Boolean, isSynchronised: Boolean, isReserved: Boolean) = {
        val mock = mockFunction[IO[swaydb.Error.Segment, Int]]
        val rootCache = getTestCache(isBlockIO, isConcurrent, isSynchronised, isReserved, stored = false, None)(_ => mock.apply())
        ...
        def doTest(blockIO: Boolean) = {
          ...
          val cache =
            if (randomBoolean())
              ...
              simpleCache.flatMap {
                Cache.concurrentIO(synchronised = false, stored = false, None)((int: Int) => IO(int))
              }
            else
              simpleCache
