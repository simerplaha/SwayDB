...
class IOEffectSpec extends TestBase {
  ...
  "fileId" should {
    "value the file id" in {
      IOEffect.fileId(Paths.get("/one/1.log")).valueIOGet shouldBe(1, Extension.Log)
      IOEffect.fileId(Paths.get("/one/two/10.log")).valueIOGet shouldBe(10, Extension.Log)
      IOEffect.fileId(Paths.get("/one/two/three/1000.seg")).valueIOGet shouldBe(1000, Extension.Seg)
    }

    "fail if the file's name is not an integer" in {
      val path = Paths.get("/one/notInt.log")
      IOEffect.fileId(path).failed.valueIOGet.exception shouldBe swaydb.Exception.NotAnIntFile(path)
    }

    "fail if the file has invalid extension" in {
      val path = Paths.get("/one/1.txt")
      IOEffect.fileId(path).failed.valueIOGet.exception shouldBe swaydb.Exception.UnknownExtension(path)
    }
  }
  ...
  "incrementFileId" should {
    "return a new file path with incremented file id" in {
      IOEffect.incrementFileId(Paths.get("/one/1.log")).valueIOGet shouldBe Paths.get("/one/2.log")
      IOEffect.incrementFileId(Paths.get("/one/two/10.log")).valueIOGet shouldBe Paths.get("/one/two/11.log")
      IOEffect.incrementFileId(Paths.get("/one/two/three/1000.seg")).valueIOGet shouldBe Paths.get("/one/two/three/1001.seg")
    }
  }
  ...
  "files" should {
    "fetch all the files in sorted order" in {
      ...
      actual.foreach {
        path =>
          IOEffect.createFile(path).valueIOGet
      }
