...
sealed trait LevelKeyValuesSpec extends TestBase with MockFactory with PrivateMethodTester {
  ...
  "putKeyValues" should {
    ...
    "fail put if writing one KeyValue fails" in {
      ...
      keyValues.add(Persistent.Put(1235, None, null, Time.empty, 10, 10, 10, 10, 10, 0, -1, false)) //give it a null Reader so that it fails reading the value.
      ...
      level.get(123).runRandomIO.right.value shouldBe empty
      level.get(1234).runRandomIO.right.value shouldBe empty
    }
  }
}
