    ...
    "return distributions and total Segments count when Segments are non-empty" in {
      ...
      val (distributions, segmentsCount) =
        PathsDistributor.getDistributions(
          ...
          () =>
            Seq(
              TestSegment(path = path1.resolve("11.seg")).runRandomIO,
              TestSegment(path = path3.resolve("31.seg")).runRandomIO,
              TestSegment(path = path3.resolve("32.seg")).runRandomIO
            )
        )
  ...
  "PathsDistributor.next" should {
    ...
    "return paths based on the distribution ratio and update distribution when Segments are distributed unevenly" in {
      ...
      segments expects() returning
        Seq(
          TestSegment(path = path1.resolve("1.seg")).runRandomIO,
          TestSegment(path = path2.resolve("2.seg")).runRandomIO,
          TestSegment(path = path3.resolve("3.seg")).runRandomIO
        )
      ...
      segments expects() returning
        Seq(
          TestSegment(path = path1.resolve("11.seg")).runRandomIO,
          TestSegment(path = path2.resolve("22.seg")).runRandomIO,
          TestSegment(path = path2.resolve("222.seg")).runRandomIO,
          TestSegment(path = path2.resolve("2222.seg")).runRandomIO,
          TestSegment(path = path2.resolve("22222.seg")).runRandomIO,
          TestSegment(path = path2.resolve("222222.seg")).runRandomIO
        )
      ...
      segments expects() returning
        Seq(
          TestSegment(path = path1.resolve("111.seg")).runRandomIO,
          TestSegment(path = path2.resolve("2222222.seg")).runRandomIO,
          TestSegment(path = path2.resolve("22222222.seg")).runRandomIO,
          TestSegment(path = path3.resolve("33.seg")).runRandomIO,
          TestSegment(path = path3.resolve("333.seg")).runRandomIO,
          TestSegment(path = path3.resolve("3333.seg")).runRandomIO
        )
