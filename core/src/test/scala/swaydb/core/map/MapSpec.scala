...
class MapSpec extends TestBase {
  ...
  "Map" should {
    ...
    "initialise a persistent Appendix map and recover from it when it's contains data" in {
      ...
      val segment1 = TestSegment(Slice(Transient.put(1, Some(1), None), Transient.put(2, Some(2), None)).updateStats).runIO
      val segment2 = TestSegment(Slice(Transient.put(3, Some(3), None), Transient.put(4, Some(4), None)).updateStats).runIO
    ...
    "initialise a Map that has two persistent Appendix map files (second file did not value deleted due to early JVM termination)" in {
      ...
      val segment1 = TestSegment(Slice(Transient.put(1, Some(1), None), Transient.put(2, Some(2), None)).updateStats).runIO
      val segment2 = TestSegment(Slice(Transient.put(2, Some(2), None), Transient.put(4, Some(4), None)).updateStats).runIO
      val segment3 = TestSegment(Slice(Transient.put(3, Some(3), None), Transient.put(6, Some(6), None)).updateStats).runIO
      val segment4 = TestSegment(Slice(Transient.put(4, Some(4), None), Transient.put(8, Some(8), None)).updateStats).runIO
      val segment5 = TestSegment(Slice(Transient.put(5, Some(5), None), Transient.put(10, Some(10), None)).updateStats).runIO
      val segment2Updated = TestSegment(Slice(Transient.put(2, Some(2), None), Transient.put(12, Some(12), None)).updateStats).runIO
  ...
  "PersistentMap.nextFile" should {
    "creates a new file from the current file" in {
      ...
      val mapEntries = MapCodec.read(nextFileBytes, dropCorruptedTailEntries = false).runIO.item.value
      ...
      (1 to 100) foreach {
        _ =>
          ...
          keyValues.groupedSlice(5) foreach {
            keyValues =>
              map.write(keyValues.toMapEntry.value).runIO shouldBe true
          }
