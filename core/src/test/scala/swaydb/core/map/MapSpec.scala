...
class MapSpec extends TestBase {
  ...
  "Map" should {
    "initialise a memory level0" in {
      ...
      map.write(MapEntry.Put(1, Memory.put(1, Some(1)))).valueIOGet shouldBe true
      map.write(MapEntry.Put(2, Memory.put(2, Some(2)))).valueIOGet shouldBe true
      map.get(1).value shouldBe Memory.put(1, Some(1))
      map.get(2).value shouldBe Memory.put(2, Some(2))

      map.hasRange shouldBe false

      map.write(MapEntry.Put[Slice[Byte], Memory.Remove](1, Memory.remove(1))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Remove](2, Memory.remove(2))).valueIOGet shouldBe true
      map.get(1).value shouldBe Memory.remove(1)
      map.get(2).value shouldBe Memory.remove(2)

      map.hasRange shouldBe false

      map.write(MapEntry.Put[Slice[Byte], Memory.Range](1, Memory.Range(1, 10, None, Value.remove(None)))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Range](11, Memory.Range(11, 20, Some(Value.put(20)), Value.update(20)))).valueIOGet shouldBe true
    ...
    "initialise a memory Appendix map" in {
      ...
      map.write(MapEntry.Remove[Slice[Byte]](1)).valueIOGet shouldBe true
      map.write(MapEntry.Remove[Slice[Byte]](2)).valueIOGet shouldBe true
    ...
    "initialise a persistent Level0 map and recover from it when it's empty" in {
      ...
      val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map.isEmpty shouldBe true
      map.close().valueIOGet
      //recover from an empty map
      val recovered = Map.persistent[Slice[Byte], Memory.SegmentResponse](map.path, mmap = true, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      recovered.isEmpty shouldBe true
      recovered.close().valueIOGet
    }
    ...
    "initialise a persistent Appendix map and recover from it when it's empty" in {
      ...
      val map = Map.persistent[Slice[Byte], Segment](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map.isEmpty shouldBe true
      map.close().valueIOGet
      //recover from an empty map
      val recovered = Map.persistent[Slice[Byte], Segment](map.path, mmap = true, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      recovered.isEmpty shouldBe true
      recovered.close().valueIOGet
    }
    ...
    "initialise a persistent Level0 map and recover from it when it's contains data" in {
      ...
      val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map.write(MapEntry.Put[Slice[Byte], Memory.Put](1, Memory.put(1, Some(1)))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Put](2, Memory.put(2, Some(2)))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Remove](2, Memory.remove(2))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 20, None, Value.update(20)))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 15, None, Value.remove(None)))).valueIOGet shouldBe true
      ...
      def doRecover(path: Path): PersistentMap[Slice[Byte], Memory.SegmentResponse] = {
        val recovered = Map.persistent[Slice[Byte], Memory.SegmentResponse](map.path, mmap = Random.nextBoolean(), flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
    ...
    "initialise a persistent Appendix map and recover from it when it's contains data" in {
      ...
      val segment1 = TestSegment(Slice(Transient.put(1, Some(1), None), Transient.put(2, Some(2), None)).updateStats).valueIOGet
      val segment2 = TestSegment(Slice(Transient.put(3, Some(3), None), Transient.put(4, Some(4), None)).updateStats).valueIOGet

      val map = Map.persistent[Slice[Byte], Segment](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map.write(MapEntry.Put[Slice[Byte], Segment](1, segment1)).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Segment](2, segment2)).valueIOGet shouldBe true
      map.write(MapEntry.Remove[Slice[Byte]](2)).valueIOGet shouldBe true
      map.get(1).value shouldBe segment1
      map.get(2) shouldBe empty

      def doRecover(path: Path): PersistentMap[Slice[Byte], Segment] = {
        val recovered = Map.persistent[Slice[Byte], Segment](map.path, mmap = Random.nextBoolean(), flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
    ...
    "initialise a Map that has two persistent Level0 map files (second file did not value deleted due to early JVM termination)" in {
      ...
      val map1 = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map1.write(MapEntry.Put(1, Memory.put(1, Some(1)))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(2, Memory.put(2, Some(2)))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(3, Memory.put(3, Some(3)))).valueIOGet shouldBe true
      map1.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 20, None, Value.update(20)))).valueIOGet shouldBe true

      val map2 = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map2.write(MapEntry.Put(4, Memory.put(4, Some(4)))).valueIOGet shouldBe true
      map2.write(MapEntry.Put(5, Memory.put(5, Some(5)))).valueIOGet shouldBe true
      map2.write(MapEntry.Put(2, Memory.put(2, Some(22)))).valueIOGet shouldBe true //second file will override 2's value to be 22
      map2.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 15, None, Value.remove(None)))).valueIOGet shouldBe true
      ...
      val map1Recovered = Map.persistent[Slice[Byte], Memory.SegmentResponse](map1.path, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      ...
      map1Recovered.path.files(Extension.Log).map(_.fileId.valueIOGet) should contain only ((2, Extension.Log))

      map1.close().valueIOGet
      map2.close().valueIOGet
      map1Recovered.close().valueIOGet
    }
    ...
    "initialise a Map that has two persistent Appendix map files (second file did not value deleted due to early JVM termination)" in {
      ...
      val segment1 = TestSegment(Slice(Transient.put(1, Some(1), None), Transient.put(2, Some(2), None)).updateStats).valueIOGet
      val segment2 = TestSegment(Slice(Transient.put(2, Some(2), None), Transient.put(4, Some(4), None)).updateStats).valueIOGet
      val segment3 = TestSegment(Slice(Transient.put(3, Some(3), None), Transient.put(6, Some(6), None)).updateStats).valueIOGet
      val segment4 = TestSegment(Slice(Transient.put(4, Some(4), None), Transient.put(8, Some(8), None)).updateStats).valueIOGet
      val segment5 = TestSegment(Slice(Transient.put(5, Some(5), None), Transient.put(10, Some(10), None)).updateStats).valueIOGet
      val segment2Updated = TestSegment(Slice(Transient.put(2, Some(2), None), Transient.put(12, Some(12), None)).updateStats).valueIOGet

      val map1 = Map.persistent[Slice[Byte], Segment](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map1.write(MapEntry.Put(1, segment1)).valueIOGet shouldBe true
      map1.write(MapEntry.Put(2, segment2)).valueIOGet shouldBe true
      map1.write(MapEntry.Put(3, segment3)).valueIOGet shouldBe true

      val map2 = Map.persistent[Slice[Byte], Segment](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map2.write(MapEntry.Put(4, segment4)).valueIOGet shouldBe true
      map2.write(MapEntry.Put(5, segment5)).valueIOGet shouldBe true
      map2.write(MapEntry.Put(2, segment2Updated)).valueIOGet shouldBe true //second file will override 2's value to be segment2Updated
      ...
      //recover map 1 and it should contain all entries of map1 and map2
      val map1Recovered = Map.persistent[Slice[Byte], Segment](map1.path, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      ...
      map2.close().valueIOGet
      map1Recovered.close().valueIOGet
      segment1.close.valueIOGet
      segment2.close.valueIOGet
      segment3.close.valueIOGet
      segment4.close.valueIOGet
      segment5.close.valueIOGet
      segment2Updated.close.valueIOGet
    }
    ...
    "fail initialise if the Map exists but recovery is not provided" in {
      ...
      Map.persistent[Slice[Byte], Memory.SegmentResponse](
        ...
        initialWriteCount = 0,
        fileSize = 1.mb
      ).failed.valueIOGet.exception shouldBe a[FileAlreadyExistsException]

      //recovers because the recovery is provided
      Map.persistent[Slice[Byte], Memory.SegmentResponse](map.path, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet
  ...
  "PersistentMap.recover" should {
    "recover from an empty PersistentMap folder" in {
      ...
      val file = PersistentMap.recover[Slice[Byte], Memory.SegmentResponse](createRandomDir, false, 4.mb, skipList, dropCorruptedTailEntries = false).valueIOGet._1.item

      file.isOpen shouldBe true
      file.isMemoryMapped.valueIOGet shouldBe false
      file.existsOnDisk shouldBe true
      file.fileSize.valueIOGet shouldBe 0
      file.path.fileId.valueIOGet shouldBe(0, Extension.Log)
    ...
    "recover from an existing PersistentMap folder" in {
      ...
      //create a map
      val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = true, flushOnOverflow = false, fileSize = 4.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map.write(MapEntry.Put(1, Memory.put(1, 1))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Remove](2, Memory.remove(2))).valueIOGet shouldBe true
      map.write(MapEntry.Put(3, Memory.put(3, 3))).valueIOGet shouldBe true
      map.write(MapEntry.Put(3, Memory.put(3, 3))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 20, None, Value.update(20)))).valueIOGet shouldBe true
      map.write(MapEntry.Put[Slice[Byte], Memory.Range](10, Memory.Range(10, 15, None, Value.remove(None)))).valueIOGet shouldBe true

      map.currentFilePath.fileId.valueIOGet shouldBe(0, Extension.Log)

      map.hasRange shouldBe true

      val skipList = new ConcurrentSkipListMap[Slice[Byte], Memory.SegmentResponse](keyOrder)
      val recoveredFile = PersistentMap.recover(map.path, false, 4.mb, skipList, dropCorruptedTailEntries = false).valueIOGet._1.item

      recoveredFile.isOpen shouldBe true
      recoveredFile.isMemoryMapped.valueIOGet shouldBe false
      recoveredFile.existsOnDisk shouldBe true
      recoveredFile.path.fileId.valueIOGet shouldBe(1, Extension.Log) //file id gets incremented on recover
    ...
    "recover from an existing PersistentMap folder when flushOnOverflow is true" in {
      ...
      //create a map
      val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = true, flushOnOverflow = true, fileSize = 1.byte, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      ...
      map.currentFilePath.fileId.valueIOGet shouldBe(5, Extension.Log)
      ...
      //reopen file
      val skipList = new ConcurrentSkipListMap[Slice[Byte], Memory.SegmentResponse](keyOrder)
      val recoveredFile = PersistentMap.recover(map.path, true, 1.byte, skipList, dropCorruptedTailEntries = false).valueIOGet._1.item
      recoveredFile.isOpen shouldBe true
      recoveredFile.isMemoryMapped.valueIOGet shouldBe true
      recoveredFile.existsOnDisk shouldBe true
      recoveredFile.path.fileId.valueIOGet shouldBe(6, Extension.Log) //file id gets incremented on recover
      ...
      val recoveredFile2 = PersistentMap.recover(map.path, true, 1.byte, skipList2, dropCorruptedTailEntries = false).valueIOGet._1.item
      recoveredFile2.isOpen shouldBe true
      recoveredFile2.isMemoryMapped.valueIOGet shouldBe true
      recoveredFile2.existsOnDisk shouldBe true
      recoveredFile2.path.fileId.valueIOGet shouldBe(7, Extension.Log) //file id gets incremented on recover
      ...
      map.close().valueIOGet
      recoveredFile.close.valueIOGet
      recoveredFile2.close.valueIOGet
    }
    ...
    "recover from an existing PersistentMap folder with empty memory map" in {
      ...
      val skipList = new ConcurrentSkipListMap[Slice[Byte], Memory.SegmentResponse](keyOrder)
      val file = PersistentMap.recover(map.path, false, 4.mb, skipList, dropCorruptedTailEntries = false).valueIOGet._1.item

      file.isOpen shouldBe true
      file.isMemoryMapped.valueIOGet shouldBe false
      file.existsOnDisk shouldBe true
      file.path.fileId.valueIOGet shouldBe(1, Extension.Log) //file id gets incremented on recover
  ...
  "PersistentMap.nextFile" should {
    "creates a new file from the current file" in {
      ...
      val currentFile = PersistentMap.recover(createRandomDir, false, 4.mb, skipList, dropCorruptedTailEntries = false).valueIOGet._1.item
      val nextFile = PersistentMap.nextFile(currentFile, false, 4.mb, skipList).valueIOGet

      val nextFileSkipList = new ConcurrentSkipListMap[Slice[Byte], Memory.SegmentResponse](keyOrder)
      val nextFileBytes = DBFile.channelRead(nextFile.path, autoClose = false).valueIOGet.readAll.valueIOGet
      val mapEntries = MapCodec.read(nextFileBytes, dropCorruptedTailEntries = false).valueIOGet.item.value
      ...
      currentFile.close.valueIOGet
      nextFile.close.valueIOGet
    }
  }
  ...
  "PersistentMap.recovery on corruption" should {
    ...
    "fail if the WAL file is corrupted and and when dropCorruptedTailEntries = false" in {
      val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, fileSize = 4.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      (1 to 100) foreach {
        i =>
          map.write(MapEntry.Put(i, Memory.put(i, i))).valueIOGet shouldBe true
      }
      map.size shouldBe 100
      val allBytes = Files.readAllBytes(map.currentFilePath)

      def assertRecover =
        Map.persistent[Slice[Byte], Memory.SegmentResponse](map.currentFilePath.getParent, mmap = false, flushOnOverflow = false, fileSize = 4.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).failed.valueIOGet.exception shouldBe a[IllegalStateException]
    ...
    "successfully recover partial data if WAL file is corrupted and when dropCorruptedTailEntries = true" in {
      ...
      Files.write(map.currentFilePath, allBytes.dropRight(1))
      val recoveredMap = Map.persistent[Slice[Byte], Memory.SegmentResponse](map.currentFilePath.getParent, mmap = false, flushOnOverflow = false, fileSize = 4.mb, initialWriteCount = 0, dropCorruptedTailEntries = true).valueIOGet.item
      (1 to 99) foreach {
        i =>
      ...
      val recoveredMap2 = Map.persistent[Slice[Byte], Memory.SegmentResponse](recoveredMap.currentFilePath.getParent, mmap = false, flushOnOverflow = false, fileSize = 4.mb, initialWriteCount = 0, dropCorruptedTailEntries = true).valueIOGet.item
  ...
  "PersistentMap.recovery on corruption" when {
    "there are two WAL files and the first file is corrupted" in {
      ...
      val map1 = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map1.write(MapEntry.Put(1, Memory.put(1, 1))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(2, Memory.put(2, 2))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(3, Memory.put(3, 3))).valueIOGet shouldBe true

      val map2 = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map2.write(MapEntry.Put(4, Memory.put(4, 4))).valueIOGet shouldBe true
      map2.write(MapEntry.Put(5, Memory.put(5, 5))).valueIOGet shouldBe true
      map2.write(MapEntry.Put(6, Memory.put(6, 6))).valueIOGet shouldBe true
      ...
      Map.persistent[Slice[Byte], Memory.SegmentResponse](map1.path, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).failed.valueIOGet.exception shouldBe a[IllegalStateException]
      ...
      val recoveredMapWith0LogCorrupted = Map.persistent[Slice[Byte], Memory.SegmentResponse](map1.path, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = true).valueIOGet
      //recovery state contains failure because the WAL file is partially recovered.
      recoveredMapWith0LogCorrupted.result.failed.valueIOGet.exception shouldBe a[IllegalStateException]
  ...
  "PersistentMap.recovery on corruption" when {
    "there are two WAL files and the second file is corrupted" in {
      ...
      val map1 = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = false, flushOnOverflow = false, 1.mb, initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
      map1.write(MapEntry.Put(1, Memory.put(1, 1))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(2, Memory.put(2))).valueIOGet shouldBe true
      map1.write(MapEntry.Put(3, Memory.put(3, 3))).valueIOGet shouldBe true
    ...
    "result in the recovered Map to have the same skipList as the Map before recovery" in {
      ...
      (1 to 100) foreach {
        _ =>
          ...
          val map = Map.persistent[Slice[Byte], Memory.SegmentResponse](createRandomDir, mmap = Random.nextBoolean(), flushOnOverflow = true, randomIntMax(1.mb), initialWriteCount = 0, dropCorruptedTailEntries = false).valueIOGet.item
          ...
          keyValues.groupedSlice(5) foreach {
            keyValues =>
              map.write(keyValues.toMapEntry.value).valueIOGet shouldBe true
          }
          ...
          map.write(updatedEntries).valueIOGet shouldBe true
