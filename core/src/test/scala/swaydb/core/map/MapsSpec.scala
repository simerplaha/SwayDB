...
class MapsSpec extends TestBase {
  ...
  "Maps.persistent" should {
    "initialise and recover on reopen" in {
      ...
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO

      maps.write(time => MapEntry.Put(1, Memory.put(1))).runRandomIO
      maps.write(_ => MapEntry.Put(2, Memory.put(2))).runRandomIO
      maps.write(_ => MapEntry.Put[Slice[Byte], Memory.Remove](1, Memory.remove(1))).runRandomIO
      ...
      val reopen = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      //adding more entries to reopened Map should contain all entries
      reopen.write(_ => MapEntry.Put(3, Memory.put(3))).runRandomIO
      reopen.write(_ => MapEntry.Put(4, Memory.put(4))).runRandomIO
      ...
      maps.close.runRandomIO
      reopen.close.runRandomIO
    }
    ...
    "delete empty maps on recovery" in {
      ...
      val reopen = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
  ...
  "Maps.memory" should {
    "initialise" in {
      ...
      map.write(_ => MapEntry.Put(1, Memory.put(1))).runRandomIO
      map.write(_ => MapEntry.Put(2, Memory.put(2))).runRandomIO
      map.write(_ => MapEntry.Put[Slice[Byte], Memory.SegmentResponse](1, Memory.remove(1))).runRandomIO
  ...
  "Maps" should {
    "initialise a new map if the current map is full" in {
      def test(maps: Maps[Slice[Byte], Memory.SegmentResponse]) = {
        maps.write(_ => MapEntry.Put(1, Memory.put(1))).runRandomIO //entry size is 40.bytes
        maps.write(_ => MapEntry.Put(2: Slice[Byte], Memory.Range(2, 2, None, Value.update(2)))).runRandomIO //another 43.bytes
        maps.queuedMapsCountWithCurrent shouldBe 1
        //another 32.bytes but map has total size of 82.bytes.
        //now since the Map is overflow a new should value created.
        maps.write(_ => MapEntry.Put[Slice[Byte], Memory.SegmentResponse](3, Memory.remove(3))).runRandomIO
      ...
      //persistent
      val path = createRandomDir
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, 40.bytes + 43.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      test(maps)
    ...
    "write a key value larger then the actual fileSize" in {
      ...
      def test(maps: Maps[Slice[Byte], Memory.SegmentResponse]) = {
        ...
        maps.write(_ => MapEntry.Put(1, Memory.put(1, largeValue))).runRandomIO //large entry
        ...
        maps.write(_ => MapEntry.Put(2, Memory.put(2, 2))).runRandomIO
        maps.queuedMapsCount shouldBe 1
        maps.queuedMapsCountWithCurrent shouldBe 2

        //a small entry of 24.bytes gets written to the same Map since the total size is 500.bytes
        maps.write(_ => MapEntry.Put[Slice[Byte], Memory.Remove](3, Memory.remove(3))).runRandomIO
        ...
        maps.write(_ => MapEntry.Put[Slice[Byte], Memory.SegmentResponse](4, Memory.remove(4))).runRandomIO
      ...
      val path = createRandomDir
      //persistent
      test(Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, fileSize = 500.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO)
      //in memory
      test(Maps.memory(500.bytes, Accelerator.brake()))

      //reopen start in recovery mode and existing maps are cached
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, fileSize = 500.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
    ...
    "recover maps in newest to oldest order" in {
      val path = createRandomDir
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      ...
      val recovered1 = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      ...
      val recovered2 = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
    ...
    "recover from existing maps" in {
      val path = createRandomDir
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      maps.write(_ => MapEntry.Put(1, Memory.put(1))).runRandomIO
      maps.write(_ => MapEntry.Put(2, Memory.put(2))).runRandomIO
      maps.write(_ => MapEntry.Put[Slice[Byte], Memory.SegmentResponse](1, Memory.remove(1))).runRandomIO

      val recoveredMaps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO.maps.asScala
    ...
    "fail recovery if one of the map is corrupted and recovery mode is ReportFailure" in {
      ...
      Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = true, 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).failed.runRandomIO.exception shouldBe a[IllegalStateException]
    }

    "continue recovery if one of the map is corrupted and recovery mode is DropCorruptedTailEntries" in {
      val path = createRandomDir
      val maps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, 100.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).runRandomIO
      maps.write(_ => MapEntry.Put(1, Memory.put(1))).runRandomIO
      maps.write(_ => MapEntry.Put(2, Memory.put(2))).runRandomIO
      maps.write(_ => MapEntry.Put(3, Memory.put(3, 3))).runRandomIO
      maps.write(_ => MapEntry.Put(4, Memory.put(4))).runRandomIO
      maps.write(_ => MapEntry.Put(5, Memory.put(5))).runRandomIO
      maps.write(_ => MapEntry.Put(6, Memory.put(6, 6))).runRandomIO

      val secondMapsPath = maps.maps.asScala.head.pathOption.value.files(Extension.Log).head
      val secondMapsBytes = Files.readAllBytes(secondMapsPath)
      Files.write(secondMapsPath, secondMapsBytes.dropRight(1))

      val recoveredMaps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, 100.bytes, Accelerator.brake(), RecoveryMode.DropCorruptedTailEntries).runRandomIO.maps.asScala
    ...
    "continue recovery if one of the map is corrupted and recovery mode is DropCorruptedTailEntriesAndMaps" in {
      ...
      maps.write(_ => MapEntry.Put(1, Memory.put(1))).runRandomIO
      maps.write(_ => MapEntry.Put(2, Memory.put(2, 2))).runRandomIO
      maps.write(_ => MapEntry.Put(3, Memory.put(3))).runRandomIO
      maps.write(_ => MapEntry.Put(4, Memory.put(4))).runRandomIO
      maps.write(_ => MapEntry.Put(5, Memory.put(5))).runRandomIO
      maps.write(_ => MapEntry.Put(6, Memory.put(6))).runRandomIO

      val secondMapsPath = maps.maps.asScala.head.pathOption.value.files(Extension.Log).head
      val secondMapsBytes = Files.readAllBytes(secondMapsPath)
      Files.write(secondMapsPath, secondMapsBytes.dropRight(1))

      val recoveredMaps = Maps.persistent[Slice[Byte], Memory.SegmentResponse](path, mmap = false, 100.bytes, Accelerator.brake(), RecoveryMode.DropCorruptedTailEntriesAndMaps).runRandomIO
    ...
    "start a new Map if writing an entry fails" in {
      ...
      maps.write(_ => MapEntry.Put(2, Memory.put(2))).failed.runRandomIO.exception shouldBe a[NoSuchFileException]
