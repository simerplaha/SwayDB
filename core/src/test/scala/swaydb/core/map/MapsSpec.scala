...
class MapsSpec extends TestBase {
  ...
  "Maps.persistent" should {
    "initialise and recover on reopen" in {
      ...
      val maps = Maps.persistent[Slice[Byte], Memory](path, mmap = true, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
      ...
      val reopen = Maps.persistent[Slice[Byte], Memory](path, mmap = false, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
    ...
    "delete empty maps on recovery" in {
      ...
      val reopen = Maps.persistent[Slice[Byte], Memory](path, mmap = true, 1.mb, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
  ...
  "Maps.memory" should {
    "initialise" in {
      ...
      map.write(MapEntry.Put[Slice[Byte], Memory](1, Memory.Remove(1))).assertGet
  ...
  "Maps" should {
    ...
    "write a key value larger then the actual fileSize" in {
      ...
      def test(maps: Maps[Slice[Byte], Memory]) = {
        ...
        maps.write(MapEntry.Put[Slice[Byte], Memory](4, Memory.Remove(4))).assertGet
      ...
      val path = createRandomDir
      //persistent
      test(Maps.persistent[Slice[Byte], Memory](path, mmap = false, fileSize = 500.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet)
      //in memory
      test(Maps.memory(500.bytes, Accelerator.brake()))

      //reopen start in recovery mode and existing maps are cached
      val maps = Maps.persistent[Slice[Byte], Memory](path, mmap = false, fileSize = 500.bytes, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
    ...
    "recover maps in newest to oldest order" in {
      val path = createRandomDir
      val maps = Maps.persistent[Slice[Byte], Memory](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
      ...
      val recovered1 = Maps.persistent[Slice[Byte], Memory](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
      ...
      val recovered2 = Maps.persistent[Slice[Byte], Memory](path, mmap = true, fileSize = 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet
    ...
    "recover from existing maps" in {
      ...
      maps.write(MapEntry.Put(1, Memory.Put(1))).assertGet
      maps.write(MapEntry.Put(2, Memory.Put(2))).assertGet
      maps.write(MapEntry.Put[Slice[Byte], Memory](1, Memory.Remove(1))).assertGet

      val recoveredMaps = Maps.persistent[Slice[Byte], Memory](path, mmap = true, 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).assertGet.maps.asScala
    ...
    "fail recovery if one of the map is corrupted and recovery mode is ReportFailure" in {
      ...
      Maps.persistent[Slice[Byte], Memory](path, mmap = true, 1.byte, Accelerator.brake(), RecoveryMode.ReportFailure).failed.assertGet shouldBe a[IllegalStateException]
    }
