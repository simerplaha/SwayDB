...
class AppendixMapEntrySpec extends TestBase {
  ...
  "MapEntryWriterAppendix & MapEntryReaderAppendix" should {
    ...
    "write Add segment" in {
      ...
      MapEntryReader.read[MapEntry.Put[Slice[Byte], Segment]](Reader[swaydb.Error.Map](slice.drop(1))).runRandomIO.value shouldBe entry

      import appendixReader.AppendixReader
      val readEntry = MapEntryReader.read[MapEntry[Slice[Byte], Segment]](Reader[swaydb.Error.Map](slice)).runRandomIO.value
    ...
    "write Remove Segment" in {
      ...
      MapEntryReader.read[MapEntry.Remove[Slice[Byte]]](Reader[swaydb.Error.Map](slice.drop(1))).runRandomIO.value.key shouldBe entry.key
    ...
    "write and remove key-value" in {
      ...
      val segment5 = TestSegment().runRandomIO
      ...
      val crcEntries = MapCodec.read[Slice[Byte], Segment](bytes, false).runRandomIO.item.value
      skipList.clear()
