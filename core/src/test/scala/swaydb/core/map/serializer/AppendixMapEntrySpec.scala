...
class AppendixMapEntrySpec extends TestBase {
  ...
  "MapEntryWriterAppendix & MapEntryReaderAppendix" should {
    ...
    "write Add segment" in {
      ...
      MapEntryReader.read[MapEntry.Put[Slice[Byte], Segment]](Reader[swaydb.Error.Map](slice.drop(1))).runIO.value shouldBe entry

      import appendixReader.AppendixReader
      val readEntry = MapEntryReader.read[MapEntry[Slice[Byte], Segment]](Reader[swaydb.Error.Map](slice)).runIO.value
    ...
    "write Remove Segment" in {
      ...
      MapEntryReader.read[MapEntry.Remove[Slice[Byte]]](Reader[swaydb.Error.Map](slice.drop(1))).runIO.value.key shouldBe entry.key
    ...
    "write and remove key-value" in {
      ...
      val segment3 = TestSegment().runIO
      val segment4 = TestSegment().runIO
      val segment5 = TestSegment().runIO
      ...
      val crcEntries = MapCodec.read[Slice[Byte], Segment](bytes, false).runIO.item.value
      skipList.clear()
