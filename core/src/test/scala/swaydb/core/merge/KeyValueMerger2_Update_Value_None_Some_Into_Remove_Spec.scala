...
class KeyValueMerger2_Update_Value_None_Some_Into_Remove_Spec extends WordSpec with Matchers with CommonAssertions {
    ...
    "Remove(None)" in {
      (1 to 20) foreach {
        i =>
          ...
          (Memory.Update(1, None, deadline), Memory.Remove(1, None)).merge shouldBe Memory.Remove(1, None)
  ...
  "Update(None, Some) -> Remove(Some)" when {
    "Remove(HasTimeLeft-Greater)" in {
      ...
      (Memory.Update(1, None, deadline), Memory.Remove(1, deadline2)).merge shouldBe Memory.Update(1, None, deadline)
    }
    ...
    "Remove(HasNoTimeLeft-Greater)" in {
      ...
      (Memory.Update(1, None, deadline), Memory.Remove(1, deadline2)).merge shouldBe Memory.Update(1, None, deadline2)
    }
    ...
    "Remove(Expired-Greater)" in {
      ...
      (Memory.Update(1, None, deadline), Memory.Remove(1, deadline2)).merge shouldBe Memory.Remove(1, deadline2)
    }
