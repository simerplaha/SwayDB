...
class KeyValueMerger2_Update_Value_Some_None_Into_Remove_Spec extends WordSpec with Matchers with CommonAssertions {
  ...
  "Update(Some, None) -> Remove(None)" when {
    "Remove(None)" in {
      (Memory.Update(1, 1, None), Memory.Remove(1, None)).merge shouldBe Memory.Remove(1, None)
    }
  }
