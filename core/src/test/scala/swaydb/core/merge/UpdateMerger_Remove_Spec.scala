...
class UpdateMerger_Remove_Spec extends WordSpec with Matchers {
  ...
  "Merging update into Remove" when {
    ...
    "times are in order" in {
      ...
      runThis(1000.times) {
        ...
        val oldKeyValue = randomRemoveKeyValue(key = key)(eitherOne(timeGenerator, TestTimeGenerator.Empty))
