  ...
  "return Some" when {
    ...
    "1a" in {
      runThis(100.times) {
        ...
        val upperLevel = randomRangeKeyValue(0, 10, fromValue, randomRangeValue(addRemoves = false, deadline = randomDeadlineOption(false), functionOutput = randomFunctionOutput(false)))
