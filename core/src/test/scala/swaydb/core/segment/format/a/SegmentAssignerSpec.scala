...
sealed trait SegmentAssignerSpec extends TestBase {
  ...
  "SegmentAssign.assign" should {
    ...
    "assign KeyValues to the first Segment if there is only one Segment" in {
      ...
      val segment = TestSegment().runRandomIO

      val result = SegmentAssigner.assign(keyValues, List(segment)).runRandomIO
    ...
    "assign a KeyValue and then a Group that spread over multiple Segments" in {
      ...
      val segment1 = TestSegment(randomKeyValues(startId = Some(1))).runRandomIO
      val segment2 = TestSegment(randomKeyValues(startId = Some(10))).runRandomIO

      val result = SegmentAssigner.assign(newKeyValues, List(segment1, segment2)).runRandomIO
      result.size shouldBe 2
    }

    "assign KeyValues to second Segment when none of the keys belong to the first Segment" in {
      val segment1 = TestSegment(Slice(Transient.put(1), Transient.Range.create[FromValue, RangeValue](2, 10, None, Value.remove(10.seconds.fromNow))).updateStats).runRandomIO
      val segment2 = TestSegment(Slice(Transient.put(10)).updateStats).runRandomIO
    ...
    "assign gap KeyValue to the first Segment if the first Segment already has a key-value assigned to it" in {
      val segment1 = TestSegment(Slice(randomFixedKeyValue(1), randomRangeKeyValue(2, 10)).toTransient).runRandomIO
      val segment2 = TestSegment(Slice(randomFixedKeyValue(20)).toTransient).runRandomIO
      val segments = Seq(segment1, segment2)
      ...
      val result = SegmentAssigner.assign(keyValues, segments).runRandomIO
      result.size shouldBe 1
    ...
    "assign gap KeyValue to the second Segment if the first Segment has no key-value assigned to it" in {
      runThis(10.times) {
        ...
        val segment1 = TestSegment(segment1KeyValues.toTransient).runRandomIO
        val segment2 = TestSegment(segment2KeyValues.toTransient).runRandomIO
        val segments = Seq(segment1, segment2)
    ...
    "assign gap Range KeyValue to all Segments that fall within the Range's toKey" in {
      // 1 - 10(exclusive)
      val segment1 = TestSegment(Slice(Transient.put(1), Transient.Range.create[FromValue, RangeValue](2, 10, None, Value.remove(None))).updateStats).runRandomIO
      // 20 - 20
      val segment2 = TestSegment(Slice(Transient.remove(20)).updateStats).runRandomIO
      //21 - 30
      val segment3 = TestSegment(Slice(Transient.Range.create[FromValue, RangeValue](21, 30, None, Value.remove(None)), Transient.put(30)).updateStats).runRandomIO
      //40 - 60
      val segment4 = TestSegment(Slice(Transient.remove(40), Transient.Range.create[FromValue, RangeValue](41, 50, None, Value.remove(None)), Transient.put(60)).updateStats).runRandomIO
      //70 - 80
      val segment5 = TestSegment(Slice(Transient.put(70), Transient.remove(80)).updateStats).runRandomIO
      ...
      assertResult(SegmentAssigner.assign(keyValues, segments).runRandomIO)

      //group should also result in same.
      val grouped = randomGroup(keyValues.toTransient).toMemory
      assertResult(SegmentAssigner.assign(Slice(grouped), segments).runRandomIO)
    }

    "assign key value to the first segment when the key is the new smallest" in {
      val segment1 = TestSegment(Slice(randomFixedKeyValue(1), randomFixedKeyValue(2)).toTransient).runRandomIO
      val segment2 = TestSegment(Slice(randomFixedKeyValue(4), randomFixedKeyValue(5)).toTransient).runRandomIO

      //segment1 - 1 - 2
      //segment2 - 4 - 5
      val segments = Seq(segment1, segment2)

      SegmentAssigner.assign(Slice(Memory.put(0)), segments).runRandomIO ==> {
        result =>
          result.size shouldBe 1
          result.keys.head.path shouldBe segment1.path
      }
    ...
    "assign key value to the first segment and split out to other Segment when the key is the new smallest and the range spreads onto other Segments" in {
      val segment1 = TestSegment(Slice(Transient.put(1), Transient.put(2)).updateStats).runRandomIO
      val segment2 = TestSegment(Slice(Transient.put(4), Transient.put(5)).updateStats).runRandomIO
      val segment3 = TestSegment(Slice(Transient.Range.create[FromValue, RangeValue](6, 10, Some(Value.remove(None)), Value.update(10)), Transient.remove(10)).updateStats).runRandomIO
      ...
      SegmentAssigner.assign(Slice(Memory.Range(0, 20, Some(Value.put(0)), Value.remove(None))), segments).runRandomIO ==> {
        assignments =>
          assignments.size shouldBe 3
    ...
    "debugger" in {
      val segment1 = TestSegment(Slice(Memory.put(1), Memory.Range(26074, 26075, None, Value.update(None, None))).toTransient).runRandomIO
      val segment2 = TestSegment(Slice(Memory.put(26075), Memory.Range(28122, 28123, None, Value.update(None, None))).toTransient).runRandomIO
      val segment3 = TestSegment(Slice(Memory.put(28123), Memory.Range(32218, 32219, None, Value.update(None, None))).toTransient).runRandomIO
      val segment4 = TestSegment(Slice(Memory.put(32219), Memory.Range(40410, 40411, None, Value.update(None, None))).toTransient).runRandomIO
      val segment5 = TestSegment(Slice(Memory.put(74605), Memory.put(100000)).toTransient).runRandomIO

      val segments = Seq(segment1, segment2, segment3, segment4, segment5)

      SegmentAssigner.assign(Slice(Memory.put(1), Memory.put(100000)), segments).runRandomIO ==> {
        assignments =>
    ...
    "assign key value to the last segment when the key is the new largest" in {
      val segment1 = TestSegment(Slice(Transient.put(1), Transient.put(2)).updateStats).runRandomIO
      val segment2 = TestSegment(Slice(Transient.put(4), Transient.put(5)).updateStats).runRandomIO
      val segment3 = TestSegment(Slice(Transient.put(6), Transient.put(7)).updateStats).runRandomIO
      val segment4 = TestSegment(Slice(Transient.put(8), Transient.put(9)).updateStats).runRandomIO
      val segments = Seq(segment1, segment2, segment3, segment4)

      SegmentAssigner.assign(Slice(Memory.put(10, "ten")), segments).runRandomIO ==> {
        result =>
          result.size shouldBe 1
      ...
      SegmentAssigner.assign(Slice(Memory.remove(10)), segments).runRandomIO ==> {
        result =>
          result.size shouldBe 1
          result.keys.head.path shouldBe segment4.path
      ...
      SegmentAssigner.assign(Slice(Memory.Range(10, 20, Some(Value.put(10)), Value.remove(None))), segments).runRandomIO ==> {
        result =>
          result.size shouldBe 1
          result.keys.head.path shouldBe segment4.path
      ...
      SegmentAssigner.assign(Slice(randomGroup(Slice(Memory.Range(10, 20, Some(Value.put(10)), Value.remove(None))).toTransient).toMemory), segments).runRandomIO ==> {
        result =>
          result.size shouldBe 1
          result.keys.head.path shouldBe segment4.path
    ...
    "assign all KeyValues to their target Segments" in {
      ...
      val segment1 = TestSegment(Slice(randomFixedKeyValue(key = 1)).toTransient).runRandomIO
      val segment2 = TestSegment(Slice(randomFixedKeyValue(key = 2)).toTransient).runRandomIO
      val segment3 = TestSegment(Slice(randomFixedKeyValue(key = 3)).toTransient).runRandomIO
      val segment4 = TestSegment(Slice(randomFixedKeyValue(key = 4)).toTransient).runRandomIO
      val segment5 = TestSegment(Slice(randomFixedKeyValue(key = 5)).toTransient).runRandomIO

      val segments = List(segment1, segment2, segment3, segment4, segment5)

      val result = SegmentAssigner.assign(keyValues.toMemory, segments).runRandomIO
      result.size shouldBe 5

      //sort them by the fileId, so it's easier to test
      val resultArray = result.toArray.sortBy(_._1.path.fileId.runRandomIO._1)
