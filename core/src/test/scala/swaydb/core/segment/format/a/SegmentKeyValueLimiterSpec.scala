    ...
    "not drop head Group on memory-overflow" in {
      ...
      try {
        val segment =
          ...
          )(KeyOrder.default, timeOrder, functionStore, TestLimitQueues.fileOpenLimiter, None, keyValueLimiter, SegmentIO.random).valueIOGet
  ...
  "PersistentSegment" should {
    "drop Group key-value only after it's been decompressed" in {
      ...
      try {
        ...
        val segment = TestSegment(mergedKeyValues)(KeyOrder.default, keyValueLimiter, FileLimiter.empty, timeOrder, SegmentIO.random, None).valueIOGet
