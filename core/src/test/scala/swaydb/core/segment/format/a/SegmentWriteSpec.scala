...
//              keyValue
//            case Memory.Range(fromKey, _, _, _, Some(put @ Value.Put(_, deadline, _)), _, _, _, _, _, _, _, _) if deadline.forall(_.hasTimeLeft()) =>
...
//        }
//
//        val segment = TestSegment(keyValuesWithEmptyValues)
//
