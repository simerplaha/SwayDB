  ...
  "Segment" should {
    ...
    "un-slice Segment's minKey & maxKey and also un-slice cache key-values" in {
      ...
      def doAssert(keyValues: Slice[Transient]) = {
        ...
        val readKeyValues = toPersistent(keyValues).get
        ...
        assertSegment(
          ...
          assert =
            (keyValues, segment) => {
              ...
              TestSegment(failedKeyValues, path = segment.path).failed.runIO.exception shouldBe a[FileAlreadyExistsException]
    ...
    //            resetPrefixCompressionEvery = TestData.resetPrefixCompressionEvery,
    //            minimumNumberOfKeyForHashIndex = TestData.minimumNumberOfKeysForHashIndex,
    //            allocateSpace = TestData.allocateSpace, //divide by large because key-value can contain all expired.
    //            compressDuplicateValues = TestData.compressDuplicateValues
    //          ).assertGet
    //
    //        segments.foreach(_.existsOnDisk shouldBe false)
