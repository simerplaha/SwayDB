...
class BloomFilterSpec extends TestBase {
  ...
  "toBytes & toSlice" should {
    "write bloom filter to bytes" in {
      runThis(10.times) {
        ...
        val bloom = BloomFilter.read(BloomFilter.Offset(0, filter.bytes.size), Reader(filter.bytes)).get
          ...
          keyValues =
            Slice(
              Transient.Range.create[FromValue, RangeValue](
                ...
                bloomFilterConfig = BloomFilter.Config.random.copy(falsePositiveRate = 0.001)
              )
            )
        ) shouldBe defined
      }
          ...
          keyValues =
            Slice(
              Transient.Range.create[FromValue, RangeValue](
                1,
                2,
                Some(Value.Remove(None, time.next)),
                Value.update(100),
                bloomFilterConfig = BloomFilter.Config.random.copy(falsePositiveRate = 0.001)
              )
            )
  ...
  "bloomFilter error check" in {
    ...
    runThis(5.times) {
      ...
      val bloom: BloomFilter = BloomFilter.read(BloomFilter.Offset(0, state.bytes.size), Reader(state.bytes)).get
