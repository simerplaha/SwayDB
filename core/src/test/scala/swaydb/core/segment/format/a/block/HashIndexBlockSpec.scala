...
class HashIndexBlockSpec extends TestBase {
  ...
  def assertHashIndexes(keyValues: Iterable[Transient],
                        hashIndex1: UnblockedReader[HashIndexBlock],
                        hashIndex2: UnblockedReader[HashIndexBlock])(implicit order: KeyOrder[Slice[Byte]]) = {
    keyValues foreach {
      keyValue =>
  ...
  "it" should {
    "write compressed HashIndex and result in the same as uncompressed HashIndex" in {
      runThis(50.times) {
        ...
        val uncompressedBlockReader: UnblockedReader[HashIndexBlock] = Block.unblock(uncompressedHashIndex, SegmentBlock.unblocked(uncompressedState.bytes), randomBoolean()).get
