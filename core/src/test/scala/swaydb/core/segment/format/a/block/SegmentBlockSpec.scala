...
class SegmentBlockSpec extends TestBase {
  ...
  "SegmentBlock" should {
    ...
    "converting KeyValues to bytes and execute readAll and find on the bytes" in {
      def test(keyValues: Slice[Memory]) = {
        ...
        closedSegment should have size 1

        val segmentBytes = closedSegment.head.flattenSegmentBytes
  ...
  //          keyValues.exists {
  //            case _: Memory.Remove => true
  //            case _: Memory.Put => false
  //            case _: Memory.Update => false
  //            case _: Memory.Function => true
  ...
  //      def doAssert(_keyValues: Slice[Memory]) = {
