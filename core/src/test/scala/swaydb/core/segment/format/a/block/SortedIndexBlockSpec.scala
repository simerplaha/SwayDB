...
class SortedIndexBlockSpec extends TestBase with PrivateMethodTester {
  ...
  "write, close, readAll & search" in {
    runThis(50.times, log = true) {
      ...
      val offset = SortedIndexBlock.Offset(0, closedState.bytes.size)
      val block = SortedIndexBlock.read(offset, SegmentBlock.unblocked(closedState.bytes)).get
