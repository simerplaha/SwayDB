  ...
  "Range into Single" when {
        ...
        val expectedKeyValue =
          (newKeyValue.fromValue, newKeyValue.rangeValue) match {
            ...
            case (Some(Value.Remove(None, _)), Value.Remove(None, _)) =>
              newKeyValue
            case _ =>
              Memory.Range(1, 10, expectedFromValue.toFromValue().valueIOGet, newKeyValue.rangeValue)
          }
    ...
    "Mid - When Single's key-value belongs to Range's fromKey to toKey" in {
      runThis(10000.times) {
        ...
        val merged = FixedMerger(newKeyValue.rangeValue.toMemory(midKey), oldKeyValue).valueIOGet
          ...
          newKeyValue.rangeValue match {
            ...
            case _ =>
              Slice(
                ...
                Memory.Range(midKey, 10, merged.toFromValue().valueIOGet, newKeyValue.rangeValue)
              )
          }
