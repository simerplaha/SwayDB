        ...
        val expectedKeyValue =
          Slice(
            ...
            Memory.Range(
              fromKey = 10,
              toKey = 15,
              fromValue = oldKeyValue.fromValue.map(fromValue => FixedMerger(newKeyValue.rangeValue.toMemory(10), fromValue.toMemory(10)).flatMap(_.toFromValue()).runRandomIO),
              rangeValue = FixedMerger(newKeyValue.rangeValue.toMemory(Slice.emptyBytes), oldKeyValue.rangeValue.toMemory(Slice.emptyBytes)).flatMap(_.toRangeValue()).runRandomIO
            ),
            Memory.Range(
              fromKey = 15,
              toKey = 20,
        ...
        val expectedKeyValue =
          Slice(
            ...
            Memory.Range(
              fromKey = 10,
              toKey = 20,
              fromValue = oldKeyValue.fromValue.map(oldFromValue => FixedMerger(newKeyValue.rangeValue.toMemory(10), oldFromValue.toMemory(10)).flatMap(_.toFromValue()).runRandomIO),
              rangeValue = FixedMerger(newKeyValue.rangeValue.toMemory(Slice.emptyBytes), oldKeyValue.rangeValue.toMemory(Slice.emptyBytes)).flatMap(_.toRangeValue()).runRandomIO
            )
          )
        val expectedLastLevel = expectedKeyValue.flatMap(keyValue => keyValue.fromValue.flatMap(_.toExpectedLastLevelKeyValue(keyValue.key))).toSlice
        ...
        val from: Option[Value.FromValue] =
          oldKeyValue.fromValue match {
            case Some(oldFromValue) =>
              Some(FixedMerger(newKeyValue.fromValue.getOrElse(newKeyValue.rangeValue).toMemory(10), oldFromValue.toMemory(10)).flatMap(_.toFromValue()).runRandomIO)
            case None =>
              newKeyValue.fromValue match {
                case Some(newFromValue) =>
                  Some(FixedMerger(newFromValue.toMemory(10), oldKeyValue.fromValue.getOrElse(oldKeyValue.rangeValue).toMemory(10)).flatMap(_.toFromValue()).runRandomIO)
                case None =>
                  None
              }
          }
        ...
        val expectedKeyValue =
          Slice(
            Memory.Range(
              ...
              toKey = 15,
              fromValue = from,
              rangeValue = ValueMerger(newKeyValue.rangeValue: Value.RangeValue, oldKeyValue.rangeValue: Value.RangeValue).runRandomIO
            ),
            Memory.Range(
              fromKey = 15,
              toKey = 20,
        ...
        val expectedKeyValue =
          Slice(
            ...
            Memory.Range(
              fromKey = 11,
              toKey = 15,
              fromValue = newKeyValue.fromValue.map(fromValue => FixedMerger(fromValue.toMemory(11), oldKeyValue.rangeValue.toMemory(11)).flatMap(_.toFromValue()).runRandomIO),
              rangeValue = FixedMerger(newKeyValue.rangeValue.toMemory(Slice.emptyBytes), oldKeyValue.rangeValue.toMemory(Slice.emptyBytes)).flatMap(_.toRangeValue()).runRandomIO
            ),
            Memory.Range(
              fromKey = 15,
