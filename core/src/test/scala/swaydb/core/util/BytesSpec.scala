...
class BytesSpec extends WordSpec with Matchers {
  ...
  "compressJoin & decompressJoin" when {
    "there are no common bytes" in {
      ...
      val (readBytes1, readBytes2) = Bytes.decompressJoin(mergedBytes).valueIO.value
    ...
    "merge, compress and uncompress byte arrays" in {
      ...
      //uncompress
      individuallyCompressedBytes.map(Bytes.decompressJoin).map(_.valueIO.value).toList shouldBe keys
  ...
  "writeUnsignedIntReversed" in {
    Seq(Int.MaxValue, 100000000, 123, 0) foreach {
      intToWrite =>
        ...
        ByteUtil.readLastUnsignedInt(sliceReverse).valueIO.value shouldBe ((intToWrite, slice.size))
    }
  }
}
