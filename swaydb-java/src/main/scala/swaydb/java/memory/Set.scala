...
object Set {
  ...
  def configWithFunctions[A](serializer: JavaSerializer[A]): Config[A, swaydb.java.PureFunction[A, Void, Apply.Set[Void]], swaydb.PureFunction[A, Void, Apply.Set[Void]]] =
    new Config(
