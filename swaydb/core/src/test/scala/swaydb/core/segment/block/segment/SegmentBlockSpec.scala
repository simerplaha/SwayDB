...
class SegmentBlockSpec extends AnyWordSpec {

  val keyValueCount = 100

  private implicit def testTimer: TestTimer = TestTimer.random
  ...
  "SegmentBlock" should {
    ...
    //      //
    //      //        SegmentBlock.read(SegmentBlockOffset(0, bytes.size), bytes.createReader()) map {
    //      //          segmentBlock =>
          ...
          runThis(100.times) {
            doAssert(
              Slice(
                ...
                randomRangeKeyValue(12, 15, randomFromValue(), rangeValue = Value.remove(None))
              )
            )
          }
      }
