...
class RemoveMerger_Function_Spec extends AnyWordSpec {
        ...
        runThis(1000.times) {
          ...
          val expected =
            if (newKeyValue.deadline.isEmpty)
              ...
              Memory.PendingApply(key = key, Slice(oldKeyValue.toFromValue().runRandomIO.get, newKeyValue.toFromValue().runRandomIO.get))
