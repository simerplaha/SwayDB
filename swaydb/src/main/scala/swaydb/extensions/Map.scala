...
import swaydb.{OK, From, IO, Prepare}
...
//@formatter:on
  ...
  def put(key: K, value: V): IO.ApiIO[OK] =
    map.put(key = Key.MapEntry(mapKey, key), value = Some(value))

  def put(key: K, value: V, expireAfter: FiniteDuration): IO.ApiIO[OK] =
    map.put(Key.MapEntry(mapKey, key), Some(value), expireAfter.fromNow)

  def put(key: K, value: V, expireAt: Deadline): IO.ApiIO[OK] =
    map.put(Key.MapEntry(mapKey, key), Some(value), expireAt)

  def put(keyValues: (K, V)*): IO.ApiIO[OK] =
    put(keyValues)

  def put(keyValues: Iterable[(K, V)]): IO.ApiIO[OK] =
    map.put {
      keyValues map {
        case (key, value) =>
  ...
  def remove(key: K): IO.ApiIO[OK] =
    map.remove(Key.MapEntry(mapKey, key))

  def remove(from: K, to: K): IO.ApiIO[OK] =
    map.remove(Key.MapEntry(mapKey, from), Key.MapEntry(mapKey, to))

  def remove(keys: K*): IO.ApiIO[OK] =
    remove(keys)

  def remove(keys: Iterable[K]): IO.ApiIO[OK] =
  ...
  def expire(key: K, after: FiniteDuration): IO.ApiIO[OK] =
    map.expire(Key.MapEntry(mapKey, key), after.fromNow)

  def expire(key: K, at: Deadline): IO.ApiIO[OK] =
    map.expire(Key.MapEntry(mapKey, key), at)

  def expire(from: K, to: K, after: FiniteDuration): IO.ApiIO[OK] =
    map.expire(Key.MapEntry(mapKey, from), Key.MapEntry(mapKey, to), after.fromNow)

  def expire(from: K, to: K, at: Deadline): IO.ApiIO[OK] =
    map.expire(Key.MapEntry(mapKey, from), Key.MapEntry(mapKey, to), at)

  def expire(keys: (K, Deadline)*): IO.ApiIO[OK] =
    expire(keys)

  def expire(keys: Iterable[(K, Deadline)]): IO.ApiIO[OK] =
    map.expire(keys.map(keyDeadline => (Key.MapEntry(mapKey, keyDeadline._1), keyDeadline._2)))

  def update(key: K, value: V): IO.ApiIO[OK] =
    map.update(Key.MapEntry(mapKey, key), Some(value))

  def update(from: K, to: K, value: V): IO.ApiIO[OK] =
    map.update(Key.MapEntry(mapKey, from), Key.MapEntry(mapKey, to), Some(value))

  def update(keyValues: (K, V)*): IO.ApiIO[OK] =
    update(keyValues)

  def update(keyValues: Iterable[(K, V)]): IO.ApiIO[OK] =
    map.update {
      keyValues map {
        case (key, value) =>
          (Key.MapEntry(mapKey, key), Some(value))
  ...
  def commitPrepared(prepare: Prepare[K, V, Nothing]*): IO.ApiIO[OK] =
    this.commit(prepare)
  ...
  def commit(prepare: Iterable[Prepare[K, V, Nothing]]): IO.ApiIO[OK] =
